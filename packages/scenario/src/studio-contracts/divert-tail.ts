 ...
 *
 * A scenario transfers across maps when everything in it re-binds: lane
 * placements re-bind semantically, and interaction clips ("turn left at the
 * next junction", "stop when you reach the crosswalk") re-bind because they name
 * situations rather than coordinates. An absolute polyline re-binds to nothing —
 * `{x: 626.8, y: 1812.1}` is one build of one map, and on any other map it is a
 * point in a field.
 *
