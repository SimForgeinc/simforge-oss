...
export { vegetationLodLevels } from './vegetation-lod-levels.js';
export { browserSceneBuildKey, browserSceneFingerprint, browserSceneNativeInput, browserSceneNotApplicable, buildBrowserScene, BROWSER_SCENE_DIR, BROWSER_SCENE_REVISION, BROWSER_SCENE_SCHEMA } from './browser-scene.js';
...
export async function masterStage(options: RunMapPipelineOptions): Promise<MasterStageResult> {
  ...
  const toolFingerprint = sha256(`${sceneTool}\0sidecars=${ROAD_SIDECAR_REVISION}\0sumo=${sumoKey}\0ground=${groundKey}${lodKey}`);
