...
import { runWorkerSumo, sumoPaths, type HeadlessSumoSample } from '../sumo-headless.js';
import { resolveAmbientTrafficProfile } from '@simforge-oss/engine';

interface DebugSumoRuntime {
  ...
  readonly signalAgreement: { readonly samples: number; readonly mismatches: number };
...
interface DebugReport {
  ...
  readonly performance: {
    ...
    readonly sumo: DebugSumoRuntime | null;
  };
  readonly summary: Record<string, unknown>;
  readonly actors: Record<string, readonly DebugPathSample[]>;
  readonly ambientActors: Record<string, readonly HeadlessSumoSample[]>;
...
export async function debugScenario(options: DebugOptions): Promise<number> {
  ...
  const sumo = options.provider === 'sumo' && compiled.bundle
    ? await debugSumo(input, compiled.bundle, options, sampleSeconds)
    : null;
...
async function compareReport(
  ...
  ambientActors: Record<string, readonly HeadlessSumoSample[]>,
...
/** The worker SUMO step over this input: authored actors first, SUMO one-way. */
async function debugSumo(
  input: SimScenarioInput,
  bundle: MapBundle,
  options: DebugOptions,
  sampleSeconds: number,
): Promise<{ runtime: DebugSumoRuntime; paths: Record<string, readonly HeadlessSumoSample[]> }> {
  const started = performance.now();
  const run = await runWorkerSumo({
    // Ambient generated by the native provider is replaced by SUMO.
    input: withoutGeneratedAmbient(input),
    bundle,
    profile: resolveAmbientTrafficProfile({
      version: 1,
      preset: 'city',
      maxActors: options.ambientCount ?? 32,
      seed: options.seed ?? input.seed,
    }),
    map: { assetId: input.mapId, versionId: 'local' },
  });
  const diagnostics = run.traffic.diagnostics;
  return {
    runtime: {
      version: diagnostics.runtime.version.split('-')[0]!,
      build: diagnostics.runtime.version,
      wasmSha256: diagnostics.runtime.wasmSha256,
      key: diagnostics.key,
      milliseconds: round(performance.now() - started),
      peakActors: diagnostics.peakActors,
      uniqueActors: diagnostics.uniqueActors,
      signalAgreement: { samples: diagnostics.signalAgreement.samples, mismatches: diagnostics.signalAgreement.mismatches },
      warnings: diagnostics.warnings,
    },
    paths: sumoPaths(run, Math.max(0.02, sampleSeconds)),
  };
}

function withoutGeneratedAmbient(input: SimScenarioInput): SimScenarioInput {
  return { ...input, actors: input.actors.filter((actor) => !actor.tags.includes('ambient')) };
}
