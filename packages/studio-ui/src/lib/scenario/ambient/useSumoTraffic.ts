...
export function useSumoTraffic(
  ...
  useEffect(() => {
    ...
    run.current = active;
    // The native contact solver owns every vehicle released from SUMO; it and
    // the map's static colliders load alongside the SUMO assets, and a runtime
    // that cannot load fails the bootstrap like any other missing asset.
    const collisionRuntime = Promise.all([
      loadEngine(),
      loadStaticMapColliders(options.map.manifest),
    ]);
    void collisionRuntime.catch(() => undefined);
    setStatus({
      phase: "loading",
      actorCount: 0,
    ...
    void loadSumoAssets(
      ...
      .then(
        async ({
          ...
          const initialAuthoredActors = externals.current;
          const initialExternalActors = externalTrafficActors(
            initialAuthoredActors,
            occupancyRoads,
          );
          ...
          const [engine, colliderBundle] = await collisionRuntime;
          if (cancelled) return;
          const collision = new SumoCollisionHandoff(engine.module);
          if (colliderBundle.diagnostics.status === "ready") {
            collision.setStaticColliders(colliderBundle.colliders);
          }
          ...
          active.collision = collision;
          active.lastExternalActors = initialExternalActors;
          active.lastAuthoredActors = initialAuthoredActors;
  ...
  useEffect(() => {
    ...
    if (capture && !active.captureInvalidated && capture.nextTime === 0 && active.lastResult) {
      capture.recordProviderFrame(
        0,
        collision.composeMaterializedActors(
          decodeSumoMaterializedActors(active.lastResult),
        ),
        active.lastSignalHeads ?? {},
      );
    ...
    const previousAuthoredActors = active.lastAuthoredActors;
    const targetExternalActors = [
      ...externalTrafficActors(
        collision.composeAuthoredSources(targetAuthoredActors),
        occupancyRoads,
      ),
      ...collision.externalActors(),
    ];
    ...
    active.stepping = active.stepping
      .then(async () => {
        ...
        for (const [index, request] of requests.entries()) {
          ...
          // The handoff sees every SUMO interval with the authored poses at
          // that same instant, so the frame it composes is the one presented,
          // fed back to SUMO and recorded: never raw provider output.
          sumoViews = decodeSumoActorViews(result, options.sampleHeight!);
          released =
            collision.step(
              request.deltaSeconds,
              interpolateAuthoredActors(
                previousAuthoredActors,
          ...
          if (capture && !active.captureInvalidated && !capture.complete) {
            ...
            capture.recordProviderFrame(
              capture.nextTime,
              collision.composeMaterializedActors(decodeSumoMaterializedActors(result)),
              frameSignals,
            );
            finalizeMaterializedTrafficCapture(
              active,
        ...
        options.collisionActorOverrides?.replace(
          collision.authoredViews(options.sampleHeight!),
        );
        const collisionViews = collision.composeViews(
          sumoViews,
          options.sampleHeight!,
        );
        if (released) {
  ...
  function resetSumoRun(active: SumoTrafficRun, reset: SumoResetRequest): void {
    ...
    active.lastAuthoredActors = externals.current;
...
interface SumoTrafficRun {
  ...
  /** Authored poses at the last stepped instant; the sweep origin for intermediate catch-up intervals. */
  lastAuthoredActors: readonly SumoExternalActorView[];
  warmupSeconds?: number;
  statusBase?: Pick<
    SumoTrafficStatus,
    | "initMilliseconds"
  ...
  /** Native contact authority for released SUMO/authored vehicles; bound together with `occupancyRoads`. */
  collision: SumoCollisionHandoff | null;
}
...
/**
 * Authored poses at fraction `alpha` of the way from the last stepped instant
 * to the current one, so a multi-interval catch-up hands the collision
 * authority the same intermediate samples SUMO receives. Actors without a
 * previous sample (and the final interval) use the current pose as-is.
 */
export function interpolateAuthoredActors(
  previous: readonly SumoExternalActorView[],
  current: readonly SumoExternalActorView[],
  alpha: number,
): readonly SumoExternalActorView[] {
  if (alpha >= 1 || previous.length === 0) return current;
  const previousById = new Map(
    previous.map((actor) => [actor.id, actor] as const),
  );
  return current.map((after) => {
    ...
    return {
      ...
      x: before.x + (after.x - before.x) * alpha,
      z: before.z + (after.z - before.z) * alpha,
      headingRad: before.headingRad + headingDelta * alpha,
      speedMps: before.speedMps + (after.speedMps - before.speedMps) * alpha,
    };
  });
}
