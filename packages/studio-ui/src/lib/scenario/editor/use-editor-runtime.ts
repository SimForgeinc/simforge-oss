...
export function useEditorRuntime({
  ...
  // start the WASM engine, the lane graph and the worker's map assets while the
  // scene is still streaming, rather than queueing them behind it. See the
  // module header. `loadEngine()` is cached per thread, so this is the same
  // runtime the controller effect below receives; a failure here is retried
  // and surfaced by that effect rather than swallowed.
  useEffect(() => {
    void warmAuthoringRuntime(map, loadEngine()).catch(() => undefined);
  }, [map]);
  ...
  useEffect(() => {
    ...
    setError(null);
    void Promise.all([
      // Reuses the warmup started above rather than loading the engine or
      // fetching the topology a second time; by the time the scene is ready
      // this is already resolved. If the warmup failed it was evicted, so this
      // retries and reports the real initialization error.
      warmAuthoringRuntime(map, loadEngine()),
