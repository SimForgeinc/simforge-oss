...
const TONE_STYLES = {
  ...
  info: { container: s.info, icon: s.svgMuted },
} as const;
  ...
  return (
    <div role={tone === "info" ? undefined : "alert"} className={cn(root.className, className)} style={root.style} data-testid="refusal-notice">
      <div {...stylex.props(s.row)}>
        ...
        <div {...stylex.props(s.min0, s.flex1, s.section4)}>
          ...
          {missing.length > 0 ? <div {...stylex.props(s.textSm, s.leading6, s.textMuted)}><p>This input is missing:</p><ul {...stylex.props(s.list, s.stack1)}>{missing.map((requirement) => <li key={requirement}>{DRIVING_REQUIREMENT_LABELS[requirement]}</li>)}</ul><p>Nothing is substituted for missing driving inputs — a fabricated camera view, ego pose or label would make the numbers meaningless.</p></div> : null}
          {missingFieldPaths.length > 0 ? <div {...stylex.props(s.textSm, s.leading6, s.textMuted)}><p>The run reported these fields missing from the input:</p><ul {...stylex.props(s.list, s.stack1, s.mono, s.textXs)}>{missingFieldPaths.map((path) => <li key={path}>{path}</li>)}</ul></div> : null}
          {children}
