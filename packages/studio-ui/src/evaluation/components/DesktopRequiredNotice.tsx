...
export function DesktopRequiredNotice({ capability, description, bullets = [], downloadHref = "/download", downloadLabel = "Get the desktop app", secondaryAction, className }: {
  ...
  return <section {...mergeStyleProps(stylex.props(s.mxAuto, s.max2xl, s.section5, s.itemsStart, hairline.all, s.mutedSurface, s.p8), className)} data-testid="desktop-required-notice">
    <span {...stylex.props(s.inlineGap2, hairline.all, s.px25py1, s.textXs, s.fontMedium, s.uppercaseWide, s.textMuted)}><Monitor aria-hidden="true" {...stylex.props(s.icon14)} />Desktop app</span>
