...
export type SceneLoadProgress = { phase: SceneLoadPhase; percent: number | null; percentExact?: boolean; message: string; detail: string; download?: { transferred: string; total: string | null; speed: string; stalled: boolean; stalledFor: string | null } };
...
export function sceneLoadProgressFromSnapshot(label: string, snapshot: MapModelLoadSnapshot, tracker: SceneLoadProgressTracker): { progress: SceneLoadProgress; tracker: SceneLoadProgressTracker } {
  ...
  const parts = [snapshot.loading > 0 ? `${snapshot.loading} downloading` : null, snapshot.queued > 0 ? `${snapshot.queued} queued` : null, snapshot.uploading > 0 ? `${snapshot.uploading} uploading` : null, snapshot.downloads?.cachedBytes ? `${formatBytes(snapshot.downloads.cachedBytes)} reused locally` : null, snapshot.downloads && !snapshot.downloads.discoveryComplete ? "Total pending asset discovery" : null].filter(Boolean);
  return { tracker: { peakOutstanding, percent: trackerPercent }, progress: { phase: "assets", percent, percentExact: hasExactByteProgress, message: `Loading ${label} assets`, detail: download?.stalled ? `No download data received for ${download.stalledFor}. The connection may be stalled.` : parts.length > 0 ? `${parts.join(" · ")}…` : "Loading roads, buildings, and map objects…", download } };
}
function downloadProgress(downloads: NonNullable<MapModelLoadSnapshot["downloads"]>) { return { transferred: formatBytes(downloads.transferredBytes), total: downloads.totalBytes === null ? null : formatBytes(downloads.totalBytes), speed: downloads.bytesPerSecond === null ? "Measuring speed" : `${formatBytes(downloads.bytesPerSecond)}/s`, stalled: downloads.stalledForMs >= 5_000, stalledFor: downloads.stalledForMs > 0 ? formatDuration(downloads.stalledForMs) : null }; }
