...
import {
  ...
  type CarlaObjectDto,
} from "../../../lib/scenario/carla-objects";
...
import { ParkedCarsPanel } from "../../../lib/scenario/parking/ParkedCarsPanel";
import type { ParkedCarPlan } from "../../../lib/scenario/parking/fill";
...
export function ActorLibraryRail({
  ...
  // The floating timeline owns the bottom of the viewport. Reserve its measured
  // height for both the resting icon rail and an open catalog: at short viewport
  // heights the rail can then scroll above the dock instead of placing its last
  // tools behind it. The dock is content-driven and user-resizable, so a fixed
  // clearance would immediately become stale.
  useEffect(() => {
    ...
    resizeObserver = typeof ResizeObserver === "function"
      ? new ResizeObserver(syncDock)
      : null;

    syncDock();
    // The rail mounts before streamed map data makes the timeline eligible to
    // render. Watch that one lifecycle boundary instead of assuming the dock
    // already exists during this effect.
    const mountObserver = typeof MutationObserver === "function"
      ? new MutationObserver(syncDock)
      : null;
    mountObserver?.observe(window.document.body, { childList: true, subtree: true });
