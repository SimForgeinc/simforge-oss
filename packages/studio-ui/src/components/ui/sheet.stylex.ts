...
export const styles = stylex.create({
  overlay: {
    position: "fixed", inset: 0, zIndex: layers.dialog, backgroundColor: "rgba(0, 0, 0, 0.8)",
    animationDuration: motion.durBase, animationTimingFunction: motion.easeOut,
    ...stateMotion,
  },
  content: {
    position: "fixed", zIndex: layers.dialog, display: "flex", flexDirection: "column", gap: "1rem",
  ...
  close: {
    ...
    outline: "none", transitionProperty: "opacity", transitionDuration: motion.durBase,
    ":hover": { opacity: 1 }, ":focus": { boxShadow: `0 0 0 2px ${colors.ring}, 0 0 0 4px ${colors.bg}` },
  ...
  title: { fontSize: text.sizeLg, fontWeight: text.weightSemibold, color: colors.text },
  description: { fontSize: text.sizeSm, color: colors.mutedForeground },
