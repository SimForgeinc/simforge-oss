...
check carries `detail` and usually `fix`. Common first-run fails: no Vulkan/Metal
adapter, no ffmpeg (only needed for video), sky assets missing. Use
...
simforge maps pull richmond-field-station@v2        # the map the package names (see `package inspect`)
simforge assets pull                                # the actor models + ATTRIBUTION.json
simforge render ws --preset training --rig rig.json --out out
```

- `simforge package inspect scene.scenario.zip` shows which map release and actor
  closure the package pins. A thin package (the default export) needs both
  pulled; a full package carries them.
...
| `--registry URL` |  | Registry to probe. Default: SIMFORGE_MAPS_REGISTRY, SIMFORGE_MAPS_PUBLIC_URL, then the public registry |
