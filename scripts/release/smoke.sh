...
case "$check" in
  ...
  installer)
    ...
    docker run --rm ubuntu:24.04 sh -c "set -e; apt-get update -qq; DEBIAN_FRONTEND=noninteractive apt-get install -y -qq curl ca-certificates xz-utils mesa-vulkan-drivers libvulkan1 ffmpeg >/dev/null; curl --proto '=https' --tlsv1.2 -LsSf '$installer_url' | CARGO_HOME=/opt/simforge sh -s -- --no-modify-path >/dev/null; /opt/simforge/bin/simforge doctor" \
  ...
  container)
    ...
    docker run --rm -e SIMFORGE_DEVICE=cpu "$ref" doctor >"$work/doctor-image.json" || fail "doctor in the image (lavapipe)"
