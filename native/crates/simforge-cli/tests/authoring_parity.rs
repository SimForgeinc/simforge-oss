...
fn rewrite(value: Value, from: &str, to: &str) -> Value {
    match value {
        ...
        Value::Array(items) => {
            Value::Array(items.into_iter().map(|v| rewrite(v, from, to)).collect())
        }
...
/// Platform inputs stored under another name here (the SDK boundary refuses
/// any `package.json`): `(argv path, file under INPUTS)`.
const RENAMED_INPUTS: &[(&str, &str)] = &[("packages/cli/package.json", "npm-manifest.json.txt")];

/// An argv entry as run here: platform-only inputs come from [`INPUTS`].
fn sdk_arg(arg: &str) -> String {
    if let Some((_, file)) = RENAMED_INPUTS.iter().find(|(from, _)| *from == arg) {
        return format!("{INPUTS}{file}");
    }
...
fn authoring_commands_match_their_parity_goldens() {
    ...
    for golden_file in &files {
        ...
        let result = RENAMED_INPUTS.iter().fold(result, |r, (from, file)| {
            rewrite(r, &format!("{INPUTS}{file}"), from)
        });
    ...
    assert!(
        failures.is_empty(),
        "{} of {} cases differ from their goldens: {failures:?}",
        failures.len(),
        files.len()
    );
}
