...
use simforge_authoring::batch::{
    run_batch, BatchOptions, AMBIENT_PRESETS, DEFAULT_AMBIENT_SETTLE_S,
};
...
fn ambient_profile(args: &BatchArgs) -> Result<Option<AmbientTrafficProfile>, CliError> {
    ...
    if !AMBIENT_PRESETS.contains(&preset) {
        return Err(CliError::new(
            "bad_value",
            format!("--ambient must be {}", AMBIENT_PRESETS.join(" | ")),
        )
        .with_path("--ambient"));
    }
    ...
    serde_json::from_value(profile).map(Some).map_err(|e| {
        CliError::new("bad_value", format!("ambient profile: {e}")).with_path("--ambient")
    })
}
