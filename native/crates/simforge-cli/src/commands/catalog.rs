...
fn create(args: CreateArgs) -> CmdResult {
    ...
    if args.map.is_some() && args.maps.is_some() {
        return Err(CliError::new("bad_value", "catalog create accepts only one of --map <id> or --maps a,b,c")
            .with_path("--map"));
    }
    ...
    .map_err(cli_error)?;
    Ok(Outcome { value: summary, exit: Exit::Ok })
}
...
fn verify(args: VerifyArgs) -> CmdResult {
    ...
    .map_err(cli_error)?;
    Ok(Outcome { value: payload, exit: exit_for(ok) })
}
...
fn js_number(raw: &str) -> f64 {
    ...
    let radix = |digits: &str, base: u32| u64::from_str_radix(digits, base).map_or(f64::NAN, |v| v as f64);
    match t.get(..2) {
...
fn number_flag(name: &str, raw: Option<&str>) -> Result<Option<f64>, CliError> {
    ...
    if !value.is_finite() {
        return Err(CliError::new("bad_value", format!("--{name} must be a number, got \"{raw}\"")).with_path(format!("--{name}")));
    }
    Ok(Some(value))
}
...
fn int_flag(name: &str, raw: Option<&str>) -> Result<Option<f64>, CliError> {
    ...
    if let Some(v) = value {
        if v.fract() != 0.0 {
            ...
            return Err(CliError::new("bad_value", format!("--{name} must be an integer, got \"{shown}\"")).with_path(format!("--{name}")));
        }
    }
    Ok(value)
}
...
fn batch(args: BatchArgs) -> CmdResult {
    ...
    let (payload, ok) = catalog_batch(
        ...
        &CatalogBatchOptions {
            ...
            filter,
            trivial_ttc_s,
            collision_policy: if args.allow_collisions { CollisionPolicy::Allow } else { CollisionPolicy::Reject },
            require_full_coverage: args.require_full_coverage,
        },
    )
    .map_err(cli_error)?;
