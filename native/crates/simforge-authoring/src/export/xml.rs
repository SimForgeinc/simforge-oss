...
use super::{
    analyze_capabilities, actor_backends, finite, identifier, issue, items, js_num, js_string, merge_warnings, num,
    physics_of, resolve_scenario, stringify, text, xml_escape as xml, AsamFormat, AsamIssue, ExportDocument,
    ExportFailure, ExportOptions, ExportResult, Profile, Resolved,
};
...
fn at_f64(track: &JsValue, key: &str, index: usize) -> Option<f64> {
    track.get(key).and_then(JsValue::as_array).and_then(|a| a.get(index)).and_then(JsValue::as_f64)
}
...
fn trajectory_xml(actor_id: &str, trace: &JsValue, warmup: f64) -> String {
    ...
    let vertices: Vec<String> = t
        ...
        .map(|(index, time)| {
            [
                format!(r#"<Vertex time="{}">"#, finite(time.as_f64().unwrap_or(f64::NAN) + warmup)),
                format!(
                    "  <Position>{}</Position>",
                    world_position(
                ...
                format!(r#"  <Motion speed_longitudinal="{}"/>"#, finite(motion_speed(index))),
                "</Vertex>".to_owned(),
            ]
            .join("\n")
        })
        .collect();
    let mut out = vec![
        format!(r#"<Trajectory name="{}" closed="false">"#, xml(&identifier("trajectory", actor_id))),
...
fn compact_signal_tracks(input: &JsValue, trace: &JsValue) -> ExportResult<JsValue> {
    ...
    for program in items(input, "signalPrograms") {
        ...
        let phases = trace.at(&["ticks", "signals", id, "phase"]).and_then(JsValue::as_array);
        ...
        let changes: Vec<JsValue> = phases
            ...
            .map(|(i, state)| {
                JsValue::Object(vec![
                    ("t".into(), JsValue::Number(t[i].as_f64().unwrap_or(f64::NAN) + warmup)),
        ...
        out.push(JsValue::Object(vec![
            ...
            (
                "headIds".into(),
                program.at(&["mapBinding", "headIds"]).cloned().unwrap_or(JsValue::Array(Vec::new())),
            ),
...
fn actor_entity(actor: &JsValue, name: &str) -> String {
    ...
    let tags: Vec<&str> = items(actor, "tags").iter().filter_map(JsValue::as_str).collect();
    let mut properties = vec![
        format!(r#"<Property name="uniscenarios.actorId" value="{}"/>"#, xml(text(actor, "id"))),
        format!(r#"<Property name="uniscenarios.actorKind" value="{}"/>"#, xml(kind)),
    ];
    ...
    properties.extend(tags.iter().map(|t| format!(r#"<Property name="uniscenarios.tag" value="{}"/>"#, xml(t))));
    ...
    if matches!(kind, "pedestrian" | "sidewalk_robot" | "drone" | "animal") {
        let category = if kind == "animal" { "animal" } else { "pedestrian" };
        let mass = match kind {
            "animal" => 40,
            "sidewalk_robot" => 70,
            "drone" => 12,
        ...
        let mut out = vec![
            ...
            format!(r#"  <Pedestrian name="uniscenarios_{kind}" mass="{mass}" pedestrianCategory="{category}">"#),
            lines(&bounding_box(actor), 4),
            "    <Properties>".into(),
        ];
        props(&mut out);
        out.extend(["    </Properties>".into(), "  </Pedestrian>".into(), "</ScenarioObject>".into()]);
        return out.join("\n");
    }
    if kind == "static_object" {
        ...
        props(&mut out);
        out.extend(["    </Properties>".into(), "  </MiscObject>".into(), "</ScenarioObject>".into()]);
        return out.join("\n");
    }
    ...
    let (l, w, h) = (num(actor, &["dims", "l"]), num(actor, &["dims", "w"]), num(actor, &["dims", "h"]));
    ...
    props(&mut out);
    out.extend(["    </Properties>".into(), "  </Vehicle>".into(), "</ScenarioObject>".into()]);
    out.join("\n")
}
...
fn occluder_entity(o: &JsValue) -> String {
    ...
    let mut out = vec![
        format!(r#"<ScenarioObject name="{}">"#, xml(&name)),
        r#"  <MiscObject mass="1" name="uniscenarios_occluder" miscObjectCategory="obstacle">"#.to_owned(),
        "    <BoundingBox>".into(),
        format!(r#"      <Center x="0" y="0" z="{}"/>"#, finite(h / 2.0)),
        format!(
            r#"      <Dimensions width="{}" length="{}" height="{}"/>"#,
        ...
        "    </BoundingBox>".into(),
        "    <Properties>".into(),
        format!(r#"      <Property name="uniscenarios.occluderId" value="{}"/>"#, xml(text(o, "id"))),
    ];
    if let Some(group) = o.get("groupId").and_then(JsValue::as_str).filter(|g| !g.is_empty()) {
        out.push(format!(r#"      <Property name="uniscenarios.occluderGroupId" value="{}"/>"#, xml(group)));
    }
    out.extend(["    </Properties>".into(), "  </MiscObject>".into(), "</ScenarioObject>".into()]);
    out.join("\n")
}
...
fn vehicle_light_action(light: &str, mode: &str) -> String {
    let flashing = if mode == "flashing" { r#" flashingOnDuration="0.5" flashingOffDuration="0.5""# } else { "" };
    [
        "<PrivateAction>".to_owned(),
...
fn door_animation_action(component: &str, value: &str) -> Option<String> {
    ...
    let duration = if value == "opening" || value == "closing" { 1 } else { 0 };
    Some(
        [
            "<PrivateAction>".to_owned(),
            "  <AppearanceAction>".into(),
...
fn set_appearance_actions(input: &JsValue, interaction: &JsValue) -> Result<Vec<String>, AsamIssue> {
    let actor_id = text(interaction, "actorId");
    let null = JsValue::Null;
    let actor = items(input, "actors").iter().find(|a| text(a, "id") == actor_id).unwrap_or(&null);
    let kind = text(actor, "kind");
    let id = text(interaction, "id");
    let key = interaction.at(&["target", "key"]).and_then(JsValue::as_str).unwrap_or("");
    let value = interaction.at(&["target", "value"]).cloned().unwrap_or(JsValue::Null);
    let path = format!("interactions.{id}.target.key");
    if (key.starts_with("lights.") || key.starts_with("doors.")) && matches!(kind, "pedestrian" | "animal" | "static_object") {
        return Err(issue(
    ...
    if key == "lights.indicator" {
        match &value {
            JsValue::String(s) if s == "left" => {
                return Ok(vec![vehicle_light_action("indicatorLeft", "flashing"), vehicle_light_action("indicatorRight", "off")])
            }
            JsValue::String(s) if s == "right" => {
                return Ok(vec![vehicle_light_action("indicatorLeft", "off"), vehicle_light_action("indicatorRight", "flashing")])
            }
            JsValue::String(s) if s == "hazard" => return Ok(vec![vehicle_light_action("warningLights", "flashing")]),
    ...
    if key == "lights.reverse" {
        if let JsValue::Bool(on) = value {
            return Ok(vec![vehicle_light_action("reversingLights", if on { "on" } else { "off" })]);
        }
    }
    if key == "lights.brake" {
        if let JsValue::Bool(on) = value {
            return Ok(vec![vehicle_light_action("brakeLights", if on { "on" } else { "off" })]);
        }
    }
    let door = match key {
...
fn replay_trace(typed: &SimScenarioInput, map: &MapAsset) -> ExportResult<JsValue> {
    let failed = |reason: String| ExportFailure::Unsupported(vec![issue("trajectory_replay_failed", "input", reason)]);
    ...
    let result = simforge_core::engine::run_simulation(scenario.input().clone(), options).map_err(|e| failed(e.to_string()))?;
    let feasibility_text = check_feasibility_json(&scenario, map.graph()).map_err(|e| failed(e.to_string()))?;
    let feasibility: Vec<serde_json::Value> = serde_json::from_str(&feasibility_text).unwrap_or_default();
    ...
    let errors: Vec<AsamIssue> = items_of(&issues)
        ...
        .map(|(index, i)| {
            let path = i.get("path").and_then(JsValue::as_str).map_or_else(|| format!("simulation.issues.{index}"), str::to_owned);
            issue("trajectory_replay_simulation_error", path, text(i, "reason"))
        })
        .collect();
    if !errors.is_empty() {
...
fn scheduled_event(name: &str, actions: &[String], at: f64, single: bool) -> String {
    ...
    let mut out = vec![format!(r#"<Event name="{}" priority="override" maximumExecutionCount="1">"#, xml(name))];
    if single {
        out.push(lines(&format!(r#"<Action name="{}">{}</Action>"#, xml(&format!("{name}_action")), actions[0]), 2));
    } else {
        out.extend(
            actions
                .iter()
                .enumerate()
                .map(|(i, a)| lines(&format!(r#"<Action name="{}">{a}</Action>"#, xml(&format!("{name}_action_{i}"))), 2)),
        );
    }
...
pub(crate) fn export_xml14(
    ...
    let tick_time = |index: usize| ticks_t.get(index).and_then(JsValue::as_f64).unwrap_or(f64::NAN);
    let mut issues: Vec<AsamIssue> = Vec::new();
    let mut actor_events: Vec<(String, Vec<String>)> =
        resolved.actors.iter().map(|a| (text(a.actor, "id").to_owned(), Vec::new())).collect();
    ...
    for entry in &resolved.interactions {
        ...
        let key = interaction.at(&["target", "key"]).and_then(JsValue::as_str).unwrap_or("");
        if text(interaction, "verb") != "set" || key.starts_with("rules.") || key.starts_with("signal:") {
            continue;
        }
        let id = text(interaction, "id");
        ...
        let mut event = vec![format!(r#"<Event name="{}" priority="override" maximumExecutionCount="1">"#, xml(&entry.name))];
        event.extend(actions.iter().enumerate().map(|(i, a)| {
            lines(&format!(r#"<Action name="{}">{a}</Action>"#, xml(&format!("{}_action_{i}", entry.name))), 2)
        }));
        event.push(lines(&trigger, 2));
        event.push("</Event>".into());
        push_event(&mut actor_events, text(interaction, "actorId"), event.join("\n"));
    }
    for actor in items(input, "actors") {
        ...
        let present: Vec<f64> = trace
            ...
            .collect();
        if present.first() == Some(&0.0) && present.iter().any(|p| *p == 1.0) {
            issues.push(issue(
    ...
    for (p, program) in items(input, "signalPrograms").iter().enumerate() {
        let Some(binding) = program.get("mapBinding").filter(|b| b.truthy()) else { continue };
        let id = text(program, "id");
        let phases = trace.at(&["ticks", "signals", id, "phase"]).and_then(JsValue::as_array);
    ...
    let signal_actor = items(input, "actors").first().map(|a| text(a, "id").to_owned());
    match &signal_actor {
    ...
    for resolved_actor in &resolved.actors {
        ...
        let mut private = vec![format!(r#"<Private entityRef="{}">"#, xml(&resolved_actor.name))];
    ...
    let (physics_mode, substep) = physics_of(input, &serde_json::to_value(typed.physics_mode()).ok().and_then(|v| v.as_str().map(str::to_owned)).unwrap_or_default());
        ...
        let mut rows: Vec<(String, String)> = match header.at(&["physics", "actorBackends"]) {
            Some(JsValue::Object(entries)) => entries
                ...
                .map(|(id, b)| {
                    (
                        id.clone(),
                        format!("{id}:{}:{}:{}", js_string_or_undefined(b.get("mode")), js_string_or_undefined(b.get("reason")), js_string_or_undefined(b.get("profile"))),
                    )
                })
                .collect(),
            _ => Vec::new(),
    ...
    let mut props = vec![
        ...
        property("uniscenarios.input.schemaVersion", &js_string(input.get("schemaVersion").unwrap_or(&JsValue::Null))),
        property("uniscenarios.input.seed", &xml(&js_string(input.get("seed").unwrap_or(&JsValue::Null)))),
        property("uniscenarios.physics.mode", &physics_mode),
        property("uniscenarios.physics.substepS", &finite(substep)),
        property("uniscenarios.physics.actorBackends", &xml(&actor_backends(input))),
        property(
            "uniscenarios.export.constructCapabilities.v1",
            &xml(&stringify(&JsValue::from_serialize(&report.constructs)?)),
        ),
        property("uniscenarios.trajectoryReplay.inputHash", &header_text("inputHash")),
        property("uniscenarios.trajectoryReplay.engineVersion", &header_text("engineVersion")),
        property("uniscenarios.trajectoryReplay.mapId", &header_text("mapId")),
        property("uniscenarios.trajectoryReplay.dt", &finite(num(header, &["dt"]))),
        property("uniscenarios.trajectoryReplay.warmupSeconds", &finite(num(header, &["warmupSeconds"]))),
        property("uniscenarios.trajectoryReplay.clipSeconds", &finite(num(header, &["clipSeconds"]))),
        property("uniscenarios.trajectoryReplay.physics.mode", &xml(&js_string_or_undefined(header.at(&["physics", "mode"])))),
        property("uniscenarios.trajectoryReplay.physics.substepS", &finite(num(header, &["physics", "substepS"]))),
        property("uniscenarios.trajectoryReplay.physics.solver", &xml(&js_string_or_undefined(header.at(&["physics", "solver"])))),
        property(
            "uniscenarios.trajectoryReplay.physics.solverVersion",
            &xml(&js_string_or_undefined(header.at(&["physics", "solverVersion"]))),
        ),
        property(
        ...
        property("uniscenarios.trajectoryReplay.physics.actorBackends", &xml(&replay_backends)),
        property(
        ...
        property(
            ...
            &xml(&stringify(&JsValue::Array(
                items(input, "actors").iter().map(|a| a.get("id").cloned().unwrap_or(JsValue::Null)).collect(),
            ))),
        ),
        property(
        ...
        property(
            ...
            &xml(&stringify(input.get("interactions").unwrap_or(&JsValue::Null))),
        ),
        property("uniscenarios.trajectoryReplay.events.v1", &xml(&stringify(trace.get("events").unwrap_or(&JsValue::Null)))),
        property("uniscenarios.trajectoryReplay.signals.v1", &xml(&stringify(&signal_tracks))),
        property(
            "uniscenarios.trajectoryReplay.environment.v1",
            &xml(&stringify(input.get("operationalConditions").unwrap_or(&JsValue::Null))),
        ),
        property(
            "uniscenarios.trajectoryReplay.surfacePatches.v1",
            &xml(&stringify(input.get("surfacePatches").unwrap_or(&JsValue::Null))),
        ),
        property(
    ...
    if let Some(perception) = input.get("perception").filter(|p| p.truthy()) {
        props.push(property("uniscenarios.trajectoryReplay.perception.v1", &xml(&stringify(perception))));
    }
    ...
    if let Some(JsValue::Object(tracks)) = trace.at(&["ticks", "actors"]) {
        let mut down: Vec<(&String, f64)> = tracks
            .iter()
            .filter_map(|(id, track)| track.get("downSinceS").and_then(JsValue::as_f64).map(|t| (id, t)))
            .collect();
        down.sort_by(|a, b| simforge_core::hash::cmp_locale(a.0, b.0));
        props.extend(down.into_iter().map(|(id, t)| {
            property(&format!("uniscenarios.trajectoryReplay.knockedDownAtS.{}", xml(id)), &finite(t))
        }));
    }
    if let Some(provenance) = &options.provenance {
        ...
        props.extend(sorted.into_iter().map(|(k, v)| {
            property(&format!("uniscenarios.provenance.{}", xml(k)), &xml(&js_string(v)))
        }));
    }
    for (index, criterion) in items(input, "nearMissCriteria").iter().enumerate() {
        let prefix = format!("uniscenarios.nearMiss.{index}");
        props.push(property(&format!("{prefix}.pedestrian"), &xml(text(criterion, "pedestrianId"))));
        props.push(property(&format!("{prefix}.target"), &xml(text(criterion, "targetId"))));
        props.push(property(&format!("{prefix}.clearanceM"), &finite(num(criterion, &["clearanceM"]))));
        props.push(property(
            &format!("{prefix}.toleranceM"),
            &finite(criterion.get("toleranceM").and_then(JsValue::as_f64).unwrap_or(0.15)),
        ));
        props.push(property(
            &format!("{prefix}.pass"),
            criterion.get("pass").and_then(JsValue::as_str).unwrap_or("auto"),
        ));
        if let Some(hash) = criterion.get("planHash").and_then(JsValue::as_str).filter(|h| !h.is_empty()) {
    ...
    for program in items(input, "signalPrograms") {
        ...
        let join = |key: &str| items(binding, key).iter().map(js_string).collect::<Vec<_>>().join(",");
        props.push(property(&format!("uniscenarios.signal.{id}.timingSource"), &xml(&js_string_or_undefined(binding.get("timingSource")))));
        props.push(property(&format!("uniscenarios.signal.{id}.junctionId"), &xml(&js_string_or_undefined(binding.get("junctionId")))));
        props.push(property(&format!("uniscenarios.signal.{id}.controllerIds"), &xml(&join("controllerIds"))));
        props.push(property(&format!("uniscenarios.signal.{id}.headIds"), &xml(&join("headIds"))));
        let groups = items(binding, "controllerHeadGroups")
            ...
            .map(|g| {
                format!(
                    ...
                    items(g, "headIds").iter().map(js_string).collect::<Vec<_>>().join("+")
                )
            })
            .collect::<Vec<_>>()
            .join(";");
        props.push(property(&format!("uniscenarios.signal.{id}.controllerHeadGroups"), &xml(&groups)));
    }
    ...
    let mut content: Vec<String> = vec![
        ...
        format!(
            ...
            xml(options.description.as_deref().unwrap_or("Concrete SimForge scenario instance")),
    ...
    content.extend(resolved.actors.iter().map(|a| lines(&actor_entity(a.actor, &a.name), 4)));
    content.extend(items(input, "occluders").iter().map(|o| lines(&occluder_entity(o), 4)));
    content.extend(["  </Entities>".into(), "  <Storyboard>".into(), "    <Init><Actions>".into()]);
    ...
    content.extend(["  </Storyboard>".into(), "</OpenSCENARIO>".into(), String::new()]);
...
fn lower_to_13(content: &str) -> ExportResult<String> {
    ...
    let lowered = content
        .replacen(r#"revMajor="1" revMinor="4""#, r#"revMajor="1" revMinor="3""#, 1)
        .replace(r#"vehicleCategory="other""#, r#"vehicleCategory="car""#)
        .replace(r#"vehicleCategory="heavyTruck""#, r#"vehicleCategory="truck""#)
        .replace(r#"vehicleCategory="motorcycle""#, r#"vehicleCategory="motorbike""#)
        .replace(r#"vehicleCategory="standupScooter""#, r#"vehicleCategory="motorbike""#);
    // `/\n\s*<Motion speed_longitudinal="[^"]+"\/>/g` and `/\n\s*<Interpolation\/>/g`.
    Ok(remove_line_element(&remove_line_element(&lowered, "<Motion speed_longitudinal=\"", true), "<Interpolation/>", false))
}
            ...
            if text[j..].starts_with(start) {
                ...
                let end = if with_attr {
                    ...
                    match text[rest..].find('"') {
                        Some(q) if q > 0 && text[rest + q..].starts_with("\"/>") => Some(rest + q + 3),
                        _ => None,
                    }
                } else {
                    Some(rest)
