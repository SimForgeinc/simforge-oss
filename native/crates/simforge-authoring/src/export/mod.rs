...
pub(crate) fn issue(code: &str, path: impl Into<String>, reason: impl Into<String>) -> AsamIssue {
    AsamIssue { code: code.to_owned(), path: path.into(), reason: reason.into() }
}
...
fn d(
    ...
    CapabilityEntry { path, disposition, fidelity, reason }
}
...
fn array_len(input: &JsValue, key: &str) -> usize {
    input.get(key).and_then(JsValue::as_array).map_or(0, <[JsValue]>::len)
}

fn field_has_material_value(input: &JsValue, path: &str) -> bool {
    match path {
        "physics" | "metricSubject" | "perception" => input.get(path).is_some(),
        "interactions" | "signalPrograms" | "roadControls" | "props" | "occluders" | "occlusionPairs"
        | "nearMissCriteria" => array_len(input, path) > 0,
        _ => true,
    }
}
...
fn construct(
    ...
    ConstructEntry { source_path, source_id, kind, disposition, fidelity, representation, reason }
}
...
pub(crate) fn analyze_capabilities(input: &JsValue, profile: Profile) -> (CapabilityReport, Vec<AsamWarning>) {
    let fields = capability_decisions(profile);
    let mut summary = CapabilitySummary { preserved: 0, derived: 0, extension: 0, omitted: 0 };
    for f in &fields {
        match f.disposition {
    ...
    if !replay {
        for (i, actor) in items(input, "actors").iter().enumerate() {
            if actor.at(&["initial", "laneRef"]).is_some_and(JsValue::truthy) {
                warnings.push(issue(
    ...
    let report = CapabilityReport {
        ...
        intent: if replay { "trajectory-replay" } else { "editable-semantic" },
...
const DSL_KEYWORDS: &[&str] = &[
    "action", "actor", "and", "as", "bool", "call", "cover", "default", "def", "do", "else", "emit",
    "enum", "event", "extend", "false", "float", "hard", "if", "import", "in", "inherits", "int", "is",
    "it", "keep", "list", "modifier", "not", "of", "on", "one_of", "or", "parallel", "range", "record",
    "remove_default", "scenario", "serial", "string", "struct", "true", "uint", "until", "var", "wait",
    "with",
];
...
pub(crate) fn identifier(prefix: &str, raw: &str) -> String {
    ...
    let stem = if stem.is_empty() || stem.starts_with(|c: char| c.is_ascii_digit()) || DSL_KEYWORDS.contains(&stem.as_str()) {
...
pub(crate) fn build_route(graph: &Graph, spec: &JsValue, path: &str) -> ExportResult<Result<RouteHandle, AsamIssue>> {
    match graph.route(&stringify(spec)) {
        ...
        Err(BindingError::Argument(message)) => {
            ...
            let field = |k: &str| parsed.as_ref().and_then(|p| p.get(k)).and_then(|v| v.as_str()).map(str::to_owned);
            Ok(Err(AsamIssue {
    ...
    impl<'a> Walk<'a> {
        fn visit(&mut self, interaction: &'a JsValue) -> Option<f64> {
            ...
            if self.visiting.contains(&id) {
                self.issues.push(issue("trigger_cycle", path, "after() dependency cycle is not exportable"));
                return None;
            }
            ...
            if kind == "at" {
                ...
                match self.by_id.get(parent_id.as_str()).copied() {
                    None => self.issues.push(issue("unknown_interaction", path.clone(), format!("unknown interaction {parent_id}"))),
                    Some(parent) => {
...
pub(crate) fn resolve_scenario<'a>(
    ...
    for (i, actor) in items(input, "actors").iter().enumerate() {
        ...
        if !all.insert(name.clone()) {
            issues.push(issue("identifier_collision", format!("actors.{i}.id"), format!("{id} normalizes to duplicate {name}")));
        }
    ...
    for (i, actor) in items(input, "actors").iter().enumerate() {
        ...
        let spec = actor.at(&["behavior", "route"]).cloned().unwrap_or(JsValue::Null);
        ...
        if route.length_m() <= 1e-6 {
            issues.push(issue("route_too_short", path, "ASAM routes require at least two distinct world positions"));
            continue;
        }
        let step = options.route_sample_m.unwrap_or(20.0);
        if !step.is_finite() || step <= 0.0 {
            issues.push(issue("bad_route_sample", "routeSampleM", "route sample distance must be positive"));
            continue;
        }
        let id = text(actor, "id");
    ...
    if include_static_times {
        ...
        let (times, found) = resolve_static_start_times(items(input, "interactions"), num(input, &["warmupSeconds"]));
        static_times = times;
        issues.extend(found);
    }
    ...
    Ok(Resolved { actors, interactions, actor_names })
}
...
pub(crate) fn assert_default_controller_rules(input: &JsValue) -> ExportResult<Vec<AsamWarning>> {
    ...
    for (i, actor) in items(input, "actors").iter().enumerate() {
        ...
        let reactive: Vec<&str> = ["obeySignals", "yieldToVehicles", "yieldToPedestrians", "collisionAvoidance"]
            .into_iter()
        ...
        let cruise = actor.at(&["behavior", "cruiseSpeedMps"]).and_then(JsValue::as_f64);
        let initial = num(actor, &["initial", "speedMps"]);
        let differs = match cruise {
            None => true,
            Some(c) => !((c - initial).abs() <= 1e-9),
        };
        if differs {
            issues.push(issue(
...
pub(crate) fn physics_of(input: &JsValue, typed_mode: &str) -> (String, f64) {
    ...
    let substep = input.at(&["physics", "substepS"]).and_then(JsValue::as_f64).unwrap_or(0.005);
    (mode, substep)
}
...
pub(crate) fn actor_backends(input: &JsValue) -> String {
    ...
    rows.into_iter().map(|(id, row)| format!("{id}:{row}")).collect::<Vec<_>>().join(",")
}
...
pub fn run_export(
    ...
    if let Some(manifest) = instance.document.get("manifest") {
        if let Some(JsValue::Object(key)) = manifest.get("replayKey") {
            ...
            let mut set = |k: &str, v: JsValue| match provenance.iter_mut().find(|(name, _)| name == k) {
                Some(slot) => slot.1 = v,
                None => provenance.push((k.to_owned(), v)),
            };
            set("instanceId", manifest.get("instanceId").cloned().unwrap_or(JsValue::Null));
            set("inputHash", manifest.get("inputHash").cloned().unwrap_or(JsValue::Null));
            options.provenance = Some(provenance);
        }
    }
    let input = instance.document.get("input").cloned().unwrap_or(JsValue::Null);
    let result = match format {
        AsamFormat::Xosc14 => xml::export_xml14(&input, &instance.input, &map, &options),
        AsamFormat::Xosc13Esmini => xml::export_xml13_esmini(&input, &instance.input, &map, &options),
    ...
    std::fs::write(&absolute, &document.content).map_err(|e| crate::json::io_error(&absolute, e))?;
    let report = &document.capability_report;
    let payload = JsValue::object(vec![
        ("ok".into(), JsValue::Bool(true)),
        ("format".into(), JsValue::String(document.format.as_str().into())),
        ...
        ("roundTrip".into(), JsValue::String(report.round_trip.into())),
        (
        ...
        ("mediaType".into(), JsValue::String(document.media_type.into())),
        ("out".into(), JsValue::String(absolute.display().to_string())),
        ("bytes".into(), JsValue::Number(document.content.len() as f64)),
        ("warnings".into(), JsValue::from_serialize(&document.warnings)?),
    ]);
    Ok(payload)
}
