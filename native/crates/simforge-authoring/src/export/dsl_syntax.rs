...
pub fn validate_dsl22_profile_syntax(source: &str) -> Vec<DslDiagnostic> {
    ...
    let diag = |line: usize, column: usize, reason: String| DslDiagnostic {
        line,
        column,
        reason,
    };
    ...
    for (offset, raw) in source.split('\n').enumerate() {
        ...
        if let Some(tab) = raw.find('\t') {
            diagnostics.push(diag(
                line,
                raw[..tab].chars().count() + 1,
                "tabs are not valid indentation in the concrete profile".into(),
            ));
            continue;
        }
        let Some(visible) = content_without_comment(raw) else {
            continue;
        };
        ...
        if indent % 4 != 0 {
            diagnostics.push(diag(
                line,
                1,
                "indentation must use four-space levels".into(),
            ));
        }
        match previous {
            Some((prev_indent, opens, prev_line)) if indent > prev_indent => {
                if !opens {
                    diagnostics.push(diag(
                        line,
                        1,
                        format!("unexpected indentation after line {prev_line}"),
                    ));
                }
                active.push(indent);
            }
            _ if indent < *active.last().unwrap_or(&0) => {
                ...
                if indent != *active.last().unwrap_or(&0) {
                    diagnostics.push(diag(
                        line,
                        1,
                        "dedent does not match an enclosing block".into(),
                    ));
                }
            }
            _ => {}
        ...
        if text.starts_with("import ") {
            if indent != 0 || saw_scenario {
                diagnostics.push(diag(
                    line,
                    1,
                    "imports must precede the top-level scenario".into(),
                ));
            }
            saw_import = true;
            ...
            if indent != 0 || saw_scenario {
                diagnostics.push(diag(
                    line,
                    1,
                    "exactly one top-level scenario is allowed".into(),
                ));
            }
            saw_scenario = true;
            ...
            diagnostics.push(diag(
                line,
                1,
                "scenario declaration must precede scenario members".into(),
            ));
        }
...
mod tests {
    ...
    fn accepts_the_emitted_profile_and_rejects_the_rest() {
        ...
        assert!(
            validate_dsl22_profile_syntax(ok).is_empty(),
            "{:?}",
            validate_dsl22_profile_syntax(ok)
        );
