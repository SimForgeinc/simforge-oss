...
use super::{
    actor_backends, analyze_capabilities, assert_default_controller_rules, finite, flag, identifier, issue, items,
    js_num, js_string, merge_warnings, num, physics_of, resolve_scenario, text, AsamFormat, AsamIssue,
    ExportDocument, ExportFailure, ExportOptions, ExportResult, Pose, Profile, Resolved, ResolvedInteraction,
};
...
fn pose_declaration(name: &str, pose: Pose) -> String {
    [
        ...
        format!("    keep(it.orientation.yaw == {}rad)", finite(pose.heading_rad)),
    ]
    .join("\n")
}
...
fn actor_declaration(actor: &JsValue, name: &str) -> String {
    ...
    out.extend([
        ...
        format!("    keep(it.bounding_box.length == {}m)", finite(num(actor, &["dims", "l"]))),
        format!("    keep(it.bounding_box.width == {}m)", finite(num(actor, &["dims", "w"]))),
    ...
    if kind != "static_object" {
        ...
        out.push(format!("    keep(it.intended_infrastructure == [{infrastructure}])"));
    }
    out.join("\n")
}
...
fn path_declaration(name: &str, points: &[Pose]) -> String {
    ...
    out.push(format!(
        ...
        (0..points.len()).map(|i| pose_name(name, i)).collect::<Vec<_>>().join(", ")
    ));
    out.join("\n")
}
...
fn occluder_declaration(o: &JsValue) -> String {
    ...
    [
        ...
        "    keep(it.bounding_box.center.y == 0m)".into(),
        format!("    keep(it.bounding_box.center.z == {}m)", finite(h / 2.0)),
        format!("    keep(it.bounding_box.length == {}m)", finite(num(o, &["obb", "lengthM"]))),
        format!("    keep(it.bounding_box.width == {}m)", finite(num(o, &["obb", "widthM"]))),
        format!("    keep(it.bounding_box.height == {}m)", finite(h)),
        "    keep(it.center_of_gravity.x == 0m)".into(),
        "    keep(it.center_of_gravity.y == 0m)".into(),
        format!("    keep(it.center_of_gravity.z == {}m)", finite(h / 2.0)),
        pose_declaration(
            &pose,
            Pose { x: num(o, &["obb", "center", "x"]), z: num(o, &["obb", "center", "z"]), heading_rad: num(o, &["obb", "headingRad"]) },
        ),
...
fn initial_actor_branch(actor: &JsValue, name: &str, route_name: &str, clip: f64) -> String {
    ...
    [
        "serial:".to_owned(),
        format!("    {name}.assign_position(position: {}.position)", pose_name(route_name, 0)),
        format!("    {name}.assign_orientation(orientation: {}.orientation)", pose_name(route_name, 0)),
        format!("    {name}.assign_speed(speed: {}mps)", finite(num(actor, &["initial", "speedMps"]))),
        indent(&motion.join("\n"), 4),
    ]
    .join("\n")
}
...
fn speed_action(interaction: &JsValue, actor_name: &str, current: Option<f64>) -> Result<(String, f64), AsamIssue> {
    let target = target_speed(interaction, current)?;
    let id = text(interaction, "id");
    let dynamics = interaction.get("dynamics").cloned().unwrap_or(JsValue::Null);
    let shape = text(&dynamics, "shape");
    if shape == "step" || current == Some(target) {
        return Ok((format!("{actor_name}.assign_speed(speed: {}mps)", finite(target)), target));
    }
    if shape != "linear" {
        return Err(issue(
...
fn interaction_action(
    ...
    let id = text(interaction, "id");
    let actor_name = resolved.actor_names.get(text(interaction, "actorId")).cloned().unwrap_or_else(|| "undefined".into());
    let dynamics = interaction.get("dynamics").cloned().unwrap_or(JsValue::Null);
    let target = interaction.get("target").cloned().unwrap_or(JsValue::Null);
...
fn validate_dsl_profile(input: &JsValue) -> ExportResult<()> {
    ...
    for (i, actor) in items(input, "actors").iter().enumerate() {
        ...
        if items(actor, "tags").iter().any(|t| t.as_str() == Some("motion:reverse")) {
            issues.push(issue(
...
pub(crate) fn export_dsl22(
    ...
    ordered.sort_by(|a, b| {
        ...
        ta.partial_cmp(&tb)
            ...
            .then_with(|| simforge_core::hash::cmp_locale(text(a.interaction, "id"), text(b.interaction, "id")))
    });
    let mut branches: Vec<(f64, String, String)> = Vec::new();
    for entry in ordered {
        let actor_id = text(entry.interaction, "actorId");
        let current = speed_by_actor.iter().find(|(id, _)| id == actor_id).map(|(_, s)| *s);
        match interaction_action(&resolved, entry, current) {
            ...
            Ok((text_out, speed)) => {
                ...
                branches.push((entry.start_time_s.unwrap_or(f64::NAN), text(entry.interaction, "id").to_owned(), text_out));
            }
        }
    }
    if !issues.is_empty() {
    ...
    let road_file = options.road_file.clone().unwrap_or_else(|| format!("{}.xodr", text(input, "mapId")));
    let mut content = vec![
        ...
        format!("# uniscenarios.input.schemaVersion={}", js_string(input.get("schemaVersion").unwrap_or(&JsValue::Null))),
        format!("# uniscenarios.physics.mode={mode}"),
        format!("# uniscenarios.physics.substepS={}", js_num(substep)),
        format!("# uniscenarios.physics.actorBackends={}", actor_backends(input)),
    ];
    if let Some(provenance) = &options.provenance {
        ...
        content.extend(sorted.into_iter().map(|(k, v)| {
            format!("# uniscenarios.provenance.{k}={}", js_string(v).replace(['\r', '\n'], " "))
        }));
    }
    ...
    for a in &resolved.actors {
        ...
        if flag(a.actor, &["static"]) {
            ...
            content.push(indent(&[pose_declaration(&pose_name, pose), format!("{}.location(pose: {pose_name})", a.name)].join("\n"), 4));
        } else {
    ...
    for a in &resolved.actors {
        if !flag(a.actor, &["static"]) {
            content.push(indent(&initial_actor_branch(a.actor, &a.name, &a.route_name, clip), 8));
        }
    }
