...
const CRITERION_KEYS: &[(&str, &[&str])] = &[
    ...
    ("stationary_success", &["actorId", "window", "maxSpeedMps", "minPresentSeconds"]),
    (
        "stop_hold_resume",
        &["actorId", "window", "stopSpeedMps", "minHoldSeconds", "mustResume", "resumeMinSpeedMps", "resumeByS"],
    ),
    ("clearance", &["pair", "window", "measure", "minM"]),
    ("criticality", &["metric", "pair", "window", "minS", "maxS"]),
    ("occlusion", &["observer", "target", "occluderId", "outcome"]),
    ("lane_occupancy", &["actorId", "laneRsl", "mode", "window"]),
    ("zone_occupancy", &["actorId", "mode", "window", "zone"]),
    ("collision", &["pair", "maxCount"]),
    ("control_indication", &["signalId", "window", "mode", "indications"]),
...
fn strip_undeclared(rubric: &mut serde_json::Value) {
    retain(rubric, &["version", "intentId", "title", "originalIntent", "criteria"]);
    let Some(criteria) = rubric.get_mut("criteria").and_then(serde_json::Value::as_array_mut) else {
        return;
    };
    for criterion in criteria {
        let kind = criterion.get("kind").and_then(serde_json::Value::as_str).map(str::to_owned);
        let Some((_, keys)) = CRITERION_KEYS.iter().find(|(k, _)| Some(*k) == kind.as_deref()) else {
            continue;
        };
...
fn read_rubric(file: &std::path::Path) -> Result<String, simforge_compiler::CompileError> {
    ...
    let text = std::fs::read_to_string(file)
        .map_err(|e| CompileError::at("invalid_json", path.clone(), format!("cannot read intent rubric {path}: {e}")))?;
    ...
    match simforge_core::evaluation::intent::parse_intent_rubric(&value) {
        ...
        Err(error) => {
            ...
            Err(CompileError::at("bad_value", path, "the intent rubric is invalid")
...
pub fn run_evaluate(
    ...
    let evaluation_text = handle
        .evaluate_json(Some(&filters_json(options.filter, options.trivial_ttc_s, options.reject_collisions)))
        .map_err(trace_error)?;
    let evaluation: simforge_core::evaluation::TraceEvaluation = serde_json::from_str(&evaluation_text)
    ...
    if let Some(rubric_file) = &options.rubric {
        ...
        intent = Some(native_js(&handle.intent_rubric_json(&rubric).map_err(trace_error)?)?);
        if let Some(out) = &options.blind_review_out {
            let packet = native_js(&handle.blind_review_packet_json(&rubric).map_err(trace_error)?)?;
            crate::json::write_json_file(out, &packet)?;
        }
    } else if options.blind_review_out.is_some() {
        return Err(CompileError::at("missing_argument", "--rubric", "--blind-review-out requires --rubric"));
    }
    let intent_verdict = intent
        ...
        .map(str::to_owned);
    let combined = combined_verdict(evaluation.verdict, &evaluation.findings, intent_verdict.as_deref());
    ...
    let mut entries: Vec<(String, JsValue)> = vec![
        ...
        (
            "metricSubject".into(),
            trace.header.metric_subject.clone().map_or(JsValue::Null, JsValue::String),
        ),
        ("filter".into(), JsValue::String(options.filter.as_str().to_owned())),
        ("verdict".into(), JsValue::String(verdict_str(evaluation.verdict).to_owned())),
        ("combinedVerdict".into(), JsValue::String(combined.to_owned())),
    ...
    if let (Some(rubric), Some(intent)) = (&options.rubric, intent) {
        entries.push(("intentRubric".into(), JsValue::String(rubric.display().to_string())));
        entries.push(("intentEvaluation".into(), intent));
    }
    if let Some(out) = &options.blind_review_out {
        entries.push(("blindReviewPacket".into(), JsValue::String(out.display().to_string())));
    }
