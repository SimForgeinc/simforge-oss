...
pub fn run_simulate_instance(
    ...
    for event in JsValue::from_serialize(&result.trace.events)?.as_array().unwrap_or(&[]) {
        let kind = event.get("kind").and_then(JsValue::as_str).unwrap_or("undefined").to_owned();
    ...
    let doc = JsValue::object(vec![
        ...
        ("mapId".into(), JsValue::String(instance.input.map_id.clone())),
        ("header".into(), JsValue::from_serialize(&result.trace.header)?),
        ("traceDigest".into(), JsValue::String(digest)),
        ("metrics".into(), JsValue::from_serialize(&metrics_summary(&result.trace.metrics))?),
