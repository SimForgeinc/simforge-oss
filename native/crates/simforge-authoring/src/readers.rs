...
fn parse_input(
    ...
    file: &Path,
    reason: &str,
) -> Result<SimScenarioInput, CompileError> {
...
pub fn read_instance(file: &Path) -> Result<InstanceDoc, CompileError> {
    ...
    let (input_raw, reason) = if is_instance {
        (
            raw.get("input").unwrap_or(&null),
            "the instance does not satisfy the engine contract",
        )
    } else {
        (
            &raw,
            "the document is neither an instance nor a SimScenarioInput",
        )
    };
    ...
    let mut entries = vec![
        (
            "kind".to_owned(),
            JsValue::String("scenario-instance".into()),
        ),
    ...
    Ok(InstanceDoc {
        document: JsValue::object(entries),
        input,
    })
}
