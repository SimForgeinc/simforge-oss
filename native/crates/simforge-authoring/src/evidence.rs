...
pub fn verify_evidence_hashes(
    ...
    let input_actor_ids = js_sort(
        input
            ...
            .unwrap_or(&[])
            .iter()
            .map(|a| a.get("id").and_then(JsValue::as_str).unwrap_or("undefined").to_owned())
            .collect(),
    );
    ...
    let manifest_engine_graph_digest = string_or_null(replay_key.and_then(|k| k.get("engineGraphDigest")));
    ...
    if let Some(slot) = instance_catalog_slot {
        ...
        let invalid = js_ne(slot.get("mapId"), input_map)
            ...
            || js_ne(slot.at(&["provenance", "matcherIndexDigest"]), Some(&opt(&matcher_index_digest)))
            || js_ne(slot.at(&["provenance", "engineGraphDigest"]), Some(&opt(&manifest_engine_graph_digest)))
        ...
        if invalid {
            let expected = object(vec![
                ...
                ("engineGraphDigest", Some(opt(&manifest_engine_graph_digest))),
            ]);
            issues.push(EvidenceHashIssue {
        ...
        let manifest_variant = match operational_variant {
            Some(JsValue::Object(entries)) => JsValue::Object(
                entries.iter().filter(|(k, _)| k != "concrete").cloned().collect(),
            ),
    ...
    EvidenceHashReport {
        ...
        physics_provenance: Some(if physics_matched { "matched" } else { "mismatch" }),
        issues,
    }
}
...
pub fn run_verify(
    ...
    let ok = report.ok;
    let mut entries = vec![
        ("instance".to_owned(), JsValue::String(instance_arg.to_owned())),
