...
pub fn matcher_site_closes_location(site: &MatchedSite, value: &Js, index: &DerivedMapIndex) -> bool {
    if !value.is_record() {
        return false;
    }
    let anchor = value.get("anchor").filter(|a| a.is_record());
    let road = anchor.and_then(|a| a.get("road")).filter(|r| r.is_record());
    let scene = anchor.and_then(|a| a.get("scene")).filter(|s| s.is_record());
    ...
    let path_has_rsl = site.frame.reference_path.iter().any(|span| span.lane_rsl == rsl);
