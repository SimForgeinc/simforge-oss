...
use crate::batch::cell::{
    run_cell, CatalogProvenance, CatalogSlot, CatalogVariant, CellError, CellOptions, CellPaths,
    CellResult, CollisionPolicy,
};
...
fn artifact_provenance(slot: &Js, attempt_seed: &str, matcher_site_id: &str) -> Js {
    let mut provenance = slot
        .get("provenance")
        .cloned()
        .unwrap_or(Js::Object(Vec::new()));
    provenance.set(
        "templateDigest",
        slot.get("provenance")
            .and_then(|p| p.get("templateDigest"))
            .cloned()
            .into(),
    );
    object([
        ...
        (
            "templateId",
            slot.get("implementation")
                .and_then(|i| i.get("templateId"))
                .cloned()
                .into(),
        ),
    ])
...
fn derive_counts(slots: &[Js]) -> Js {
    ...
    object([
        ...
        (
            "supported",
            count_where(slots, |s| template_backed(s) && !state_is(s, "unsupported")).into(),
        ),
        (
            "unsupported",
            count_where(slots, |s| state_is(s, "unsupported")).into(),
        ),
        (
            "pending",
            count_where(slots, |s| state_is(s, "pending")).into(),
        ),
        (
            "running",
            count_where(slots, |s| state_is(s, "running")).into(),
        ),
        (
            "attempted",
            count_where(slots, |s| !attempts(s).is_empty()).into(),
        ),
        (
            "generated",
            count_where(slots, |s| any_attempt(s, "generated")).into(),
        ),
        (
            "simulated",
            count_where(slots, |s| any_attempt(s, "simulated")).into(),
        ),
        (
            "accepted",
            count_where(slots, |s| state_is(s, "simulated")).into(),
        ),
        (
            "rejected",
            count_where(slots, |s| state_is(s, "rejected")).into(),
        ),
        (
            "failed",
            count_where(slots, |s| state_is(s, "failed")).into(),
        ),
        (
            "resumed",
            count_where(slots, |s| truthy(s.get("resumed"))).into(),
        ),
    ])
}
...
fn reconcile_interrupted(prior: &Js) -> Js {
    ...
    let resumed = interrupted
        || ["unsupported", "simulated", "rejected", "failed"]
            .iter()
            .any(|s| state_is(prior, s));
    if interrupted {
...
fn plan_slots(root: &MapRoot, slots: &[Js]) -> Vec<Plan> {
    ...
    for slot in slots {
        ...
        if !is_str(
            implementation.and_then(|i| i.get("state")),
            "template-backed",
        ) || !truthy(source)
        {
            ...
            plans.insert(
                id.clone(),
                unsupported(
                    "unsupported_catalog_mechanism",
                    format!("incident {incident} has no executable template"),
                ),
            );
            continue;
        }
        if !truthy(implementation.and_then(|i| i.get("matcherSiteId")))
            || !strict_eq(
                implementation.and_then(|i| i.get("matchedLocationId")),
                sget(slot, &["site", "locationId"]),
            )
        {
            plans.insert(
                id.clone(),
        ...
        if !strict_eq(
            implementation.and_then(|i| i.get("materializedVariantId")),
            sget(slot, &["variant", "id"]),
        ) {
    ...
    for ((source, map_id), group) in &groups {
        ...
        let outcome = (|| -> Result<(), CompileError> {
            ...
            if let Some(bytes) = bytes {
                if !is_str(
                    sget(group[0], &["provenance", "templateDigest"]),
                    &sha256_bytes(bytes),
                ) {
            ...
            for slot in group {
                if !is_str(
                    sget(slot, &["provenance", "matcherIndexDigest"]),
                    &matcher_digest,
                ) || !is_str(
                    sget(slot, &["provenance", "engineGraphDigest"]),
                    &engine_digest,
                ) {
                    let id = identity_of(slot);
                    failed(
            ...
            if matched.report.sites.is_empty() {
                return Err(CompileError::at(
                    "no_matching_site",
                    map_id.clone(),
                    format!("template has no executable site on {map_id}"),
                )
                .detail_entry(
                    "failureSummary",
            ...
            for slot in group {
                ...
                let error = if !truthy(site_id)
                    || !strict_eq(
                        sget(slot, &["implementation", "matchedLocationId"]),
                        location_id,
                    ) {
                    Some(structured(
                        "unsupported_catalog_site_binding",
        ...
        .map(|slot| {
            plans.remove(&identity_of(slot)).unwrap_or_else(|| Plan {
                ...
                planning_error: Some(object([
                    ...
                    (
                        "reason",
                        "planning cancelled before this slot was reached".into(),
                    ),
                ])),
                unsupported_error: None,
            })
...
fn resumable_accepted(result: &Js, policy: CollisionPolicy, collisions: usize) -> bool {
    ...
    is_str(result.get("status"), "ok")
        ...
        && matches!(
            eligibility.and_then(|e| e.get("eligible")),
            Some(Js::Bool(true))
        )
        && is_str(
            eligibility.and_then(|e| e.get("collisionPolicy")),
            policy_str(policy),
        )
        && eligibility
...
impl Runner<'_> {
    ...
    fn execute(&self, plan: &Plan) -> Result<(), CompileError> {
        ...
        let template = plan
            .template
            .as_ref()
            .expect("runnable plan has a template");
        ...
        for attempt in first..max {
            ...
            let mut cell = CellOptions::new(
                &to_js_string(slot.get("mapId")),
                site_id,
                attempt as i64,
                &self.out_dir,
            );
            ...
            let mut record = vec![
                ...
                (
                    "verdict".to_owned(),
                    result.verdict.map_or(Js::Null, Js::from),
                ),
                ("generated".to_owned(), Js::from(generated)),
                ("simulated".to_owned(), Js::from(simulated)),
                (
                    "inputHash".to_owned(),
                    result.input_hash.clone().map_or(Js::Null, Js::from),
                ),
                (
                    "traceDigest".to_owned(),
                    result.trace_digest.clone().map_or(Js::Null, Js::from),
                ),
            ];
            ...
            self.with_record(&identity, |r| {
                ...
                r.set(
                    ...
                    object([
                        ...
                        ("reason", "no catalog attempt was executed".into()),
                    ]),
                );
            });
                    ...
                    final_result.error.as_ref().map_or_else(
                        || {
                            object([
                                ...
                                (
                                    "reason",
                                    "all bounded attempts failed before materialization".into(),
                                ),
                            ])
                        },
                        cell_error_js,
...
fn last_attempt(record: &[Js], f: impl Fn(&Js) -> bool) -> Option<&Js> {
...
fn valid_simulated_resume(
    ...
    let Some(last) = last_attempt(&list, |a| is_str(a.get("verdict"), "accept")) else {
        return false;
    };
    ...
    if !Path::new(&paths.instance).exists()
        || !Path::new(&paths.trace).exists()
        || !Path::new(&paths.result).exists()
    {
        return false;
    }
    (|| -> Option<bool> {
        ...
        let expected = provenance_key(Some(&artifact_provenance(
            slot,
            &to_js_string(last.get("seed")),
            &to_js_string(last.get("siteId")),
        )))?;
        ...
        Some(
            evidence_ok
                ...
                && is_str(
                    sget(&instance, &["manifest", "replayKey", "paramSeed"]),
                    &seed,
                )
                ...
                && file_sha(&paths.instance).is_some_and(|h| {
                    is_str(sget(&result, &["artifactHashes", "instanceSha256"]), &h)
                })
                && file_sha(&paths.trace)
                    .is_some_and(|h| is_str(sget(&result, &["artifactHashes", "traceSha256"]), &h))
...
fn valid_rejected_resume(
    ...
    let list = attempts(record);
    let Some(last) = last_attempt(&list, |a| truthy(a.get("generated"))) else {
        return false;
    };
    let paths = evidence_paths(evidence_root, slot);
    ...
    (|| -> Option<bool> {
        ...
        let trace_closes = if has_trace {
            ...
            truthy(last.get("simulated"))
                ...
                && trace
                    .digest()
                    .ok()
                    .is_some_and(|d| is_str(last.get("traceDigest"), &d))
        } else {
        ...
        Some(
            stringify(instance.get("catalogSlot")).as_deref() == Some(expected.as_str())
                && stringify(result.get("catalogSlot")).as_deref() == Some(expected.as_str())
                && strict_eq(
                    sget(&instance, &["manifest", "inputHash"]),
                    last.get("inputHash"),
                )
                && strict_eq(
                    sget(&instance, &["manifest", "replayKey", "paramSeed"]),
                ...
                && is_str(
                    sget(&result, &["artifactHashes", "instanceSha256"]),
                    &instance_hash,
                )
...
fn plan_digest(catalog: &Js, slots: &[Js], options: &CatalogBatchOptions) -> String {
    let rows: Vec<Js> = slots
        ...
        .map(|slot| {
            object([
                ...
                (
                    "incidentId",
                    sget(slot, &["scenario", "incidentId"]).cloned().into(),
                ),
    ...
    let body = object([
        (
            "namespace",
            sget(catalog, &["provenance", "namespace"]).cloned().into(),
        ),
        (
            "taxonomyDigest",
            sget(catalog, &["provenance", "taxonomyDigest"])
                .cloned()
                .into(),
        ),
        ...
        (
            "solverVersion",
            simforge_bindings_common::ENGINE_SEM_VER.into(),
        ),
        ("filter", options.filter.as_str().into()),
        (
            "collisionPolicy",
            policy_str(options.collision_policy).into(),
        ),
        (
            "trivialTtcS",
            options.trivial_ttc_s.map_or(Js::Null, Js::from),
        ),
        ("slots", Js::Array(rows)),
    ]);
...
pub fn catalog_batch(
    ...
    if !(max.fract() == 0.0 && (1.0..=100.0).contains(&max)) {
        return Err(CompileError::at(
            "bad_value",
            "--attempts",
            "--attempts must be an integer from 1 to 100",
        ));
    }
    ...
    if !validation.ok
        && validation
            .issues
            .iter()
            .any(|i| i.code != "missing_evidence")
    {
        return Err(CompileError::at(
            "invalid_catalog",
            file_arg,
            "catalog verification failed before execution",
        )
        .detail_entry(
            "issues",
            Value::Array(validation.issues.iter().map(|i| i.to_value()).collect()),
        )
        .as_findings());
    }
    ...
    let wanted = |list: &[String], v: Option<&Js>| {
        list.is_empty()
            || v.and_then(Js::as_str)
    ...
    if selected.is_empty() {
        return Err(CompileError::at(
            "empty_selection",
            file_arg,
            "catalog filters selected no slots",
        )
        .as_findings());
    }
    ...
    let existing = if options.force {
        ...
        std::fs::read(&ledger_file)
            .ok()
            .and_then(|b| Js::parse(&String::from_utf8_lossy(&b)).ok())
    };
    let reusable = existing.as_ref().is_some_and(|e| {
        is_str(e.get("kind"), LEDGER_KIND) && is_str(e.get("planDigest"), &digest)
    });
    ...
    let records: Vec<Js> = selected
        ...
        .map(|slot| {
            ...
            match previous.iter().rev().find(|p| {
                p.get("identity")
                    .is_some_and(|v| to_js_string(Some(v)) == id)
            }) {
                ...
                None => object([
                    ...
                    (
                        "templateId",
                        sget(slot, &["implementation", "templateId"])
                            .cloned()
                            .unwrap_or(Js::Null),
                    ),
                    ("state", "pending".into()),
                ...
                if valid_rejected_resume(&record, slot, &evidence_root, options.collision_policy) {
                    ...
                    invalidate_stale_resume(
                        ...
                        structured(
                            "stale_resume_artifact",
                            Some(&id),
                            "recorded rejection artifacts are missing",
                        ),
                    );
                    runner.set_status(&mut state, &id, "authored")?;
                }
...
mod tests {
    ...
    fn attempt_seeds() {
        ...
        assert_eq!(
            catalog_attempt_seed("abc", 2),
            sha256("abc\0replacement\x002")
        );
    }
    ...
    fn provenance_compares_independent_of_key_order() {
        ...
        let written = Js::parse(
            r#"{"identity":"x","seed":"s","variant":{"id":"v","title":"t"},"mapId":"m"}"#,
        )
        .unwrap();
        let from_trace = Js::parse(
            r#"{"identity":"x","mapId":"m","seed":"s","variant":{"title":"t","id":"v"}}"#,
        )
        .unwrap();
        assert_ne!(written.stringify(), from_trace.stringify());
        assert_eq!(
            provenance_key(Some(&written)),
            provenance_key(Some(&from_trace))
        );
        let other = Js::parse(
            r#"{"identity":"y","mapId":"m","seed":"s","variant":{"title":"t","id":"v"}}"#,
        )
        .unwrap();
