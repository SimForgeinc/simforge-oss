...
pub fn design_digest(slot: &Js) -> String {
    sha256(
        &slot
            .without(&["designDigest", "status", "acceptance"])
            .stringify(),
    )
}
...
fn evidence_paths(evidence_root: &str, map_id: &str, identity: &str) -> Js {
    ...
    object([
        ...
        (
            "renderManifest",
            format!("{base}/render/manifest.json").into(),
        ),
        ("frame", format!("{base}/render/frame.png").into()),
        ("video", format!("{base}/render/video.mp4").into()),
        (
            "visualInspection",
            format!("{base}/render/visual-inspection.json").into(),
        ),
    ])
}
...
pub fn progress_for(slots: &[Js], target: f64) -> Result<Js, CompileError> {
    ...
    let at_least = |set: &[&str]| {
        statuses
            .iter()
            .filter(|s| s.is_some_and(|s| set.contains(&s)))
            .count()
    };
    Ok(object([
        ("target", target.into()),
        ...
        (
            "generated",
            at_least(&["generated", "simulated", "rendered", "visually-accepted"]).into(),
        ),
        (
            "simulated",
            at_least(&["simulated", "rendered", "visually-accepted"]).into(),
        ),
        (
            "rendered",
            at_least(&["rendered", "visually-accepted"]).into(),
        ),
...
fn read_map_context(
    ...
    map_id: &str,
    asset: Arc<MapAsset>,
) -> Result<MapContext, CompileError> {
    ...
    let invalid =
        |reason: String| CompileError::at("invalid_map_provenance", dir_text.clone(), reason);
    ...
    if !valid {
        return Err(invalid(format!(
            "{map_id} is missing stable map/location provenance"
        ))
    ...
    if let Some(derived_asset) = derived.get("mapAssetId") {
        if !js::strict_eq(Some(derived_asset), catalog_map_asset) {
            return Err(invalid(format!(
                "{map_id} map asset IDs disagree across provenance domains"
            )));
        }
    }
    ...
    if matcher_index_digest.is_empty() || engine_graph_digest.is_empty() {
        return Err(invalid(format!(
            "{map_id} lacks concrete matcher/engine replay digests"
        )));
    }
    let provenance = object([
        ...
        (
            "catalogRevision",
            catalog.get("catalogRevision").cloned().into(),
        ),
        ("matcherIndexDigest", matcher_index_digest.into()),
        ("engineGraphDigest", engine_graph_digest.into()),
        (
            "locationCatalogDigest",
            sha256_bytes(&location_plain).into(),
        ),
...
fn string_items(value: Option<&Js>) -> Js {
    Js::Array(
        value
            ...
            .map(|a| {
                a.iter()
                    .filter(|v| matches!(v, Js::String(_)))
                    .cloned()
                    .collect()
            })
            .unwrap_or_default(),
...
fn site_score(location: &Js, incident: &Incident) -> f64 {
    ...
    let preferred = incident
        ...
        .count() as f64;
    let quality = location.get("quality");
    let exact = if is_str(quality.and_then(|q| q.get("anchor")), "exact") {
        2.0
    } else {
        0.0
    };
...
fn eligible_on_map<'a>(
    ...
    let incidents: Vec<&Incident> = taxonomy()
        ...
        .filter(|incident| {
            incident
                .map_ids
                .as_ref()
                .is_none_or(|ids| ids.contains(&context.map_id))
                && incident
    ...
    let work = |incident: &'a Incident| -> Result<Option<Eligible<'a>>, CompileError> {
        ...
        locations.sort_by(|l, r| {
            desc(site_score(l, incident), site_score(r, incident))
                .then_with(|| cmp_locale(&to_js_string(l.get("id")), &to_js_string(r.get("id"))))
        });
        let executable = executables[incident
            .implementation_template_id
            .as_ref()
            .expect("filtered")];
        let matched = match_on_map(&executable.template, bundle, &options)?
            .report
            .sites;
        ...
        candidates.sort_by(|l, r| {
            ...
            .then_with(|| {
                cmp_locale(
                    &to_js_string(l.location.get("id")),
                    &to_js_string(r.location.get("id")),
                )
            })
            .then_with(|| cmp_locale(&l.site.site_id, &r.site.site_id))
        });
        Ok((!candidates.is_empty()).then_some(Eligible {
            incident,
            candidates,
        }))
    };
...
pub fn create_catalog(
    ...
    let evidence_root =
        assert_relative_root(options.evidence_root.as_deref().unwrap_or("evidence"))?;
    ...
    for context in &contexts {
        ...
        for ordinal in 0..CATALOG_SLOTS_PER_MAP {
                ...
                coverage
                    ...
                    .then_with(|| {
                        if breadth_pending {
                            selected_on_map
                                .contains(l)
                                .cmp(&selected_on_map.contains(r))
                        } else {
                            std::cmp::Ordering::Equal
                        }
                    })
            ...
            'selection: for (index, _) in &order {
                ...
                let executable = by_registry[entry
                    .incident
                    .implementation_template_id
                    .as_ref()
                    .expect("eligible")];
                for candidate in &entry.candidates {
                    ...
                    let failures = failures_by_mechanism
                        .entry(entry.incident.id.clone())
                        .or_default();
                    ...
                    match instantiate(
                        ...
                        Ok(result) if !result.manifest.feasible => {
                            let summary = result
                                ...
                                .iter()
                                .filter(|i| {
                                    i.severity == simforge_core::error::SimIssueSeverity::Error
                                })
                            ...
                            record(
                                "manifest_infeasible",
                                format!(
                                    "{}:{site_id}:manifest_infeasible:{summary}",
                                    context.map_id
                                ),
                            );
                        }
                        ...
                        Err(error) if is_candidate_failure(&error) => {
                            record(
                                &error.code,
                                format!("{}:{site_id}:{}", context.map_id, error.code),
                            );
                        }
            ...
            let executable = by_registry[incident
                .implementation_template_id
                .as_ref()
                .expect("eligible")];
            let slot = object([
                ...
                (
                    ...
                    object([
                        ...
                        (
                            "mapCatalogRevision",
                            map.get("catalogRevision").cloned().into(),
                        ),
                        (
                            "matcherIndexDigest",
                            map.get("matcherIndexDigest").cloned().into(),
                        ),
                        (
                            "engineGraphDigest",
                            map.get("engineGraphDigest").cloned().into(),
                        ),
                        (
                            "locationCatalogDigest",
                            map.get("locationCatalogDigest").cloned().into(),
                        ),
                ...
                (
                    ...
                    object([
                        ...
                        (
                            "matchedLocationId",
                            to_js_string(candidate.location.get("id")).into(),
                        ),
                ...
                (
                    "evidencePaths",
                    evidence_paths(&evidence_root, &context.map_id, &identity),
                ),
            ]);
            let digest = design_digest(&slot);
            let mut slot = slot;
    ...
    if eligibility_counts
        .iter()
        .any(|n| *n < CATALOG_MIN_INCIDENT_TYPES_PER_MAP)
    {
        return Err(findings(
            "insufficient_map_authorability",
            dev_assets_arg,
    ...
    let missing: Vec<&Incident> = tax
        .incidents
        .iter()
        .filter(|i| !coverage.contains(&i.id))
        .collect();
    ...
    let manifest = object([
        ...
        (
            ...
            object([
                ...
                (
                    "minimumIncidentTypesPerMap",
                    CATALOG_MIN_INCIDENT_TYPES_PER_MAP.into(),
                ),
        ...
        (
            "maps",
            Js::Array(contexts.iter().map(|c| c.provenance.clone()).collect()),
        ),
        ("researchSources", tax.sources.clone()),
        ("taxonomy", tax.incidents_js.clone()),
        (
            "templates",
            Js::Array(executables.iter().map(|e| e.provenance.clone()).collect()),
        ),
...
pub fn catalog_summary(catalog: &Js, manifest_path: &Path) -> Value {
    ...
    let slots = catalog
        ...
        .cloned()
        .unwrap_or_default();
    let on = |map_id: &str| -> Vec<&Js> {
        slots
            .iter()
            .filter(|s| is_str(s.get("mapId"), map_id))
            .collect()
    };
    ...
    for map_id in &supported {
        ...
        let incidents: BTreeSet<String> = on(map_id)
            ...
            .filter_map(|s| {
                s.get("scenario")
                    .and_then(|x| x.get("incidentId"))
                    .map(|v| to_js_string(Some(v)))
            })
            .collect();
        incidents_by_map.insert(map_id.clone(), json!(incidents.len()));
    }
    let taxonomy_rows = catalog
        .get("taxonomy")
        .and_then(Js::as_array)
        .cloned()
        .unwrap_or_default();
...
pub fn write_js_file(file: &Path, value: &Js) -> Result<(), CompileError> {
    ...
    std::fs::write(file, format!("{}\n", value.stringify_pretty()))
        .map_err(|e| crate::json::io_error(file, e))
}
...
mod tests {
    ...
    fn compiler_finding_codes_are_complete() {
        ...
        let mut builder_codes: Vec<String> = vec![
            "arrival_unconverged".into(),
            "actor_unknown".into(),
            "near_miss_unsolvable".into(),
        ];
        ...
        let dynamic: Vec<(&str, Vec<String>)> =
            vec![("builder.rs", builder_codes), ("situation.rs", Vec::new())];
            ...
            for (at, _) in text.match_indices(".as_findings()") {
                ...
                match literal(&text[start..]) {
                    ...
                    None => {
                        ...
                        assert!(
                            listed.is_some(),
                            "{name}: .as_findings() on a non-literal code; list it in `dynamic`"
                        );
        ...
        assert_eq!(
            dynamic_sites, 3,
            "a new non-literal .as_findings() site appeared; list its codes"
        );
        codes.sort();
        codes.dedup();
        let missing: Vec<&String> = codes
            .iter()
            .filter(|c| !COMPILER_FINDING_CODES.contains(&c.as_str()))
            .collect();
        let extra: Vec<&&str> = COMPILER_FINDING_CODES
            .iter()
            .filter(|c| !codes.iter().any(|k| k == *c))
            .collect();
        assert!(
            missing.is_empty() && extra.is_empty(),
            "missing {missing:?}, stale {extra:?}; all: {codes:?}"
        );
    }
    ...
    fn identities_and_paths() {
        assert_eq!(
            catalog_identity(
                "yale-street",
                6.0,
                "intersection.left-turn-across-opposing-through",
                "0123456789abcdef"
            ),
