...
pub fn strings<S: AsRef<str>>(items: &[S]) -> Js {
    Js::Array(
        items
            .iter()
            .map(|s| Js::String(s.as_ref().to_owned()))
            .collect(),
    )
}
...
mod tests {
    ...
    fn round_trips_like_javascript() {
        ...
        let cases = [
            ...
            (
                r#"{"b":1,"10":2,"a":3,"2":4,"01":5}"#,
                r#"{"2":4,"10":2,"b":1,"a":3,"01":5}"#,
            ),
            (
                r#"[-0.69904327392578125, 5e-324, 1.7976931348623157e308]"#,
                r#"[-0.6990432739257812,5e-324,1.7976931348623157e+308]"#,
            ),
        ];
    ...
    fn js_string_conversion() {
        assert_eq!(
            to_js_string(Some(&Js::parse("[1,null,\"x\"]").unwrap())),
            "1,,x"
        );
