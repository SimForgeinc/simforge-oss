...
fn is_safe_evidence_path(value: Option<&Js>, evidence_root: &str) -> bool {
    let Some(value) = value.and_then(Js::as_str) else {
        return false;
    };
    if value.is_empty()
        || value.contains('\\')
...
pub fn validate_catalog(value: &Js, options: &VerifyOptions) -> Result<Report, CompileError> {
    ...
    let record = |v: Option<&Js>| -> Js {
        v.filter(|x| x.is_record())
            .cloned()
            .unwrap_or(Js::Object(Vec::new()))
    };
    let slots: Vec<Js> = value
        .get("slots")
        .and_then(Js::as_array)
        .cloned()
        .unwrap_or_default();
    let evidence_root = value
        .get("evidenceRoot")
        .and_then(Js::as_str)
        .unwrap_or("")
        .to_owned();
    ...
    if !is_str(value.get("kind"), CATALOG_KIND) || !version_ok {
        issues.push(
            "invalid_catalog",
            "$",
            format!("kind/version must be {CATALOG_KIND}@2"),
        );
    }
    let contract = record(value.get("contract"));
    let supported_rows: Vec<Js> = contract
        .get("supportedMaps")
        .and_then(Js::as_array)
        .cloned()
        .unwrap_or_default();
    ...
    if supported.is_empty()
        || supported.len() != supported_rows.len()
        || supported_set.len() != supported.len()
    {
        issues.push(
            "wrong_map_inventory",
    ...
    if !num_is(contract.get("slotsPerMap"), SLOTS)
        || !num_is(contract.get("totalSlots"), expected_total)
    {
        issues.with(
            "wrong_slot_count",
            "contract",
    ...
    if !num_is(
        contract.get("minimumIncidentTypesPerMap"),
        CATALOG_MIN_INCIDENT_TYPES_PER_MAP as f64,
    ...
    if value.get("evidenceRoot").and_then(Js::as_str).is_none()
        || !is_safe_evidence_path(
            Some(&Js::from(format!("{evidence_root}/probe"))),
            &evidence_root,
        )
    {
        issues.push(
            ...
            "evidenceRoot",
            "evidenceRoot must be a safe relative path",
        );
    }

    let taxonomy_rows: Vec<Js> = value
        .get("taxonomy")
        .and_then(Js::as_array)
        .cloned()
        .unwrap_or_default();
    ...
    let template_rows: Vec<Js> = value
        .get("templates")
        .and_then(Js::as_array)
        .cloned()
        .unwrap_or_default();
    ...
    for (index, row) in template_rows.iter().enumerate() {
        ...
        template_by_registry_id.insert(
            row.get("id")
                .and_then(Js::as_str)
                .unwrap_or_default()
                .to_owned(),
    ...
    if !is_str(top.get("taxonomyDigest"), &taxonomy_hash)
        || !is_str(top.get("generatorVersion"), CATALOG_GENERATOR_VERSION)
    {
        issues.push(
            "invalid_provenance",
            "provenance",
            "top-level taxonomy/generator provenance is stale",
        );
    }

    let map_rows: Vec<Js> = value
        .get("maps")
        .and_then(Js::as_array)
        .cloned()
        .unwrap_or_default();
        ...
        || map_row_ids.iter().any(|m| !supported_set.contains(m))
    {
        issues.push(
            "wrong_map_inventory",
            "maps",
            "maps[] must contain each declared supported map exactly once",
        );
    }
    ...
    let physical_root = match &options.evidence_root_override {
        ...
        None => resolve(
            options
                .manifest_file
                .as_deref()
                .unwrap_or(Path::new("catalog.json")),
        )
        .parent()
    ...
    for (index, raw) in slots.iter().enumerate() {
        ...
        match identity.and_then(Js::as_str) {
            ...
            Some(id) if identities.contains(id) => issues.push(
                "duplicate_identity",
                format!("{base}.identity"),
                format!("duplicate identity {id}"),
            ),
            Some(id) => {
        ...
        match seed.and_then(Js::as_str).filter(|_| is_hex(seed, 64)) {
            None => issues.push(
                "invalid_seed",
                format!("{base}.seed"),
                "seed must be 64 lowercase hexadecimal characters",
            ),
            Some(s) if seeds.contains(s) => issues.push(
                "duplicate_seed",
                format!("{base}.seed"),
        ...
        if let Some(map_id) = map_id.and_then(Js::as_str) {
            ...
            if let Some(n) = is_integer(ordinal) {
                map_ordinals
                    .entry(map_id.to_owned())
                    .or_default()
                    .insert((n + 0.0).to_bits());
            }
        ...
        let map = map_id
            .and_then(Js::as_str)
            .and_then(|id| map_by_id.get(id).copied());
        ...
        let provenance_ok = |map: &Js| {
            provenance.get("namespace").and_then(Js::as_str).is_some()
                ...
                && strict_eq(
                    provenance.get("mapCatalogRevision"),
                    map.get("catalogRevision"),
                )
                && strict_eq(
                    provenance.get("matcherIndexDigest"),
                    map.get("matcherIndexDigest"),
                )
                && strict_eq(
                    provenance.get("engineGraphDigest"),
                    map.get("engineGraphDigest"),
                )
                && strict_eq(
                    provenance.get("locationCatalogDigest"),
                    map.get("locationCatalogDigest"),
                )
        ...
        if checks.len() != 6
            ...
            || (is_str(status, "visually-accepted")
                && (!is_str(acceptance.get("state"), "accepted")
                    ...
                    || checks
                        .iter()
                        .any(|c| !c.is_record() || !is_str(c.get("state"), "passed"))))
        {
            issues.push(
        ...
        for key in required {
            let Some(relative) = paths.get(key).and_then(Js::as_str) else {
                continue;
            };
    ...
    let coverage = (!missing.is_empty() && !options.require_full_coverage)
        .then(|| json!({ "missing": missing }));
    ...
    match value.get("catalogDigest") {
        ...
        _ => issues.push(
            "catalog_digest_mismatch",
            "catalogDigest",
            "catalogDigest is required",
        ),
    }

    let _ = empty;
    let size = |sets: &HashMap<String, OrderedSet>, id: &str| {
        json!(sets.get(id).map_or(0, OrderedSet::len))
    };
    Ok(Report {
        ...
        incident_types_by_map: supported
            .iter()
            .map(|m| (m.clone(), size(&map_incidents, m)))
            .collect(),
        domains_by_map: supported
            .iter()
            .map(|m| (m.clone(), size(&map_domains, m)))
            .collect(),
        progress: calculated,
        evidence_checked,
        issues: issues.0,
...
fn check_slot(
    ...
    if !includes(incident.get("siteTypes"), &site_type) {
        issues.push(
            "invalid_site_binding",
            format!("{base}.site.type"),
            "site type is not applicable to incident",
        );
    }
    let affordances = site
        .get("affordances")
        .filter(|a| a.as_array().is_some())
        .cloned()
        .unwrap_or(Js::Array(Vec::new()));
    let required = incident
        .get("requiredAffordances")
        .and_then(Js::as_array)
        .cloned()
        .unwrap_or_default();
    if required
        .iter()
        .any(|entry| !includes(Some(&affordances), entry))
    {
        issues.push(
            "invalid_site_binding",
    ...
    let len_at_least =
        |v: Option<&Js>, n: usize| v.and_then(Js::as_array).is_some_and(|a| a.len() >= n);
    ...
    let authored_only = !is_str(implementation.get("state"), "authored-design")
        || has("matcherSiteId")
        || has("matchedLocationId");
    if truthy(incident.get("implementationTemplateId")) {
        if is_str(implementation.get("state"), "template-backed") {
            ...
            let registry_ok = registered.is_some_and(|row| {
                strict_eq(
                    implementation.get("templateId"),
                    row.get("runtimeTemplateId"),
            ...
            if has_binding
                && (!site_id_ok
                    || !strict_eq(
                        implementation.get("matchedLocationId"),
                        site.get("locationId"),
                    ))
            {
                issues.push(
            ...
            if has_binding
                && !strict_eq(
                    implementation.get("materializedVariantId"),
                    variant.get("id"),
                )
            {
                issues.with(
    ...
    for ((source, map_id), group) in &groups {
        let result = (|| -> Result<(), String> {
            let bytes = templates::template_bytes(source).ok_or_else(|| {
                format!("catalog template {source} is not embedded in this simforge binary")
            })?;
            ...
            for (index, slot) in group {
                ...
                if !is_str(stated, &digest) {
                    issues.with(
                        ...
                        format!(
                            "catalog template digest does not match the live executable {source}"
                        ),
                        Some(Value::String(digest.clone())),
                        val(stated),
                    );
            ...
            let matched =
                match_on_map(&template, asset.bundle(), &options).map_err(|e| e.to_string())?;
            let locations = read_locations(root, map_id)?;
            for (index, slot) in group {
                let site_id = slot
                    .get("implementation")
                    .and_then(|i| i.get("matcherSiteId"));
                let location_id = slot.get("site").and_then(|s| s.get("locationId"));
                let site = matched
                    .report
                    .sites
                    .iter()
                    .find(|s| is_str(site_id, &s.site_id));
                let location = locations.iter().find(|l| {
                    l.get("id")
                        .is_some_and(|id| strict_eq(Some(id), location_id))
                });
...
pub fn catalog_verify(
    ...
    let options = VerifyOptions {
        manifest_file: Some(file.to_path_buf()),
        ..options.clone()
    };
