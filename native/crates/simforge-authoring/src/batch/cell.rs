...
use simforge_compiler::invariants::{check_invariants, InvariantContext, InvariantResidualReport, InvariantStatus};
...
impl CellOptions {
    ...
    pub fn paths(&self) -> CellPaths {
        self.artifact_paths
            .clone()
            .unwrap_or_else(|| cell_paths(&self.out_dir, &self.map_id, &self.site_id, self.draw_index))
    }
...
pub fn hard_invariant_failures(invariants: &[InvariantResidualReport]) -> Vec<&InvariantResidualReport> {
    invariants
        .iter()
...
fn try_run_cell(
    ...
    materialize.variant = options.catalog_slot.as_ref().map(|slot| CatalogVariantApplication {
    ...
    let compiled = compile_at(&template.document, map.bundle(), &site.site_id, &materialize)?;
    ...
    if options.write_trace {
        write_trace_gz(std::path::Path::new(&paths.trace), &quantized_json(&handle)?)?;
    }
    ...
    let mut findings: Vec<CellFinding> = evaluation
        .findings
        .iter()
        .map(|f| CellFinding { code: code_str(f.code), reason: f.reason.clone() })
        .collect();
    if !evidence.ok {
    ...
    let mut hard_failure_codes: Vec<String> = evidence.issues.iter().map(|i| i.code.to_owned()).collect();
    hard_failure_codes.extend(required_failures.iter().map(|r| invariant_code(r).to_owned()));
    ...
    if required_failures.iter().any(|r| r.status == InvariantStatus::Violated) {
        tags.push("invariant_violated".to_owned());
    }
    if required_failures.iter().any(|r| r.status == InvariantStatus::Unchecked) {
    ...
    let issues = manifest
        ...
        .collect();
    let ambient = manifest.ambient.as_ref().map(|p| ambient_cell_report(p, &trace));
...
fn ambient_cell_report(provenance: &AmbientTrafficProvenance, trace: &SimTrace) -> AmbientCellReport {
    ...
    if let Some(t0) = t0 {
        for id in ambient_ids {
            let Some(track) = trace.ticks.actors.get(id) else { continue };
            ...
            let dx = track.x.get(t0).copied().unwrap_or(0.0) - subject.x.get(t0).copied().unwrap_or(0.0);
            let dy = track.y.get(t0).copied().unwrap_or(0.0) - subject.y.get(t0).copied().unwrap_or(0.0);
