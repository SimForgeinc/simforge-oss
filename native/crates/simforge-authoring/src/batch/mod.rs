...
use simforge_compiler::{
    cell_seed, params_version, resolve_ambient_traffic_profile, AmbientTrafficProfile,
    CompileError, ResolvedAmbientTrafficProfile, MATCH_SEMANTICS_VERSION,
};
...
pub fn run_batch(
    ...
    let ambient_profile_hash = match &resolved {
        None => "none".to_owned(),
        Some(r) if serde_json::to_value(r.preset).ok() == Some(Value::String("off".into())) => {
            "none".to_owned()
        }
        Some(r) => {
    ...
    let seed_root = typed
        .simulation
        .as_ref()
        .map_or(tid.as_str(), |s| s.seed.as_str());
    for (map, matched) in &matches {
        ...
        for site in &matched.report.sites {
            for draw in 0..options.draws.max(0) {
                let mut cell =
                    CellOptions::new(&matched.map_id, &site.site_id, draw, &options.out_dir);
    ...
    let digests: HashMap<&str, (&str, &str)> = matches
        ...
        .map(|(map, matched)| {
            (
                ...
                (
                    map.bundle().index().topology_digest.as_str(),
                    map.graph().topology_digest(),
                ),
            )
        })
        .collect();
    ...
    for (i, cell) in planned.iter().enumerate() {
        if !options.force {
            let (matcher_index_digest, engine_graph_digest) = digests
                .get(cell.options.map_id.as_str())
                .copied()
                .unwrap_or(("", ""));
    ...
    let numbers = |path: &[&str]| -> Vec<f64> {
        ordered
            .iter()
            .filter_map(|r| r.at(path).and_then(JsValue::as_f64))
            .collect()
    };
    ...
    payload.set(
        "summaryFile",
        JsValue::String(
            paths::resolve(Path::new(&summary_file))
                .display()
                .to_string(),
        ),
    );
