...
pub fn run_validate(
    ...
    let kind = if template_kind { "template" } else { "instance" };
    ...
    if options.tier == 1 {
        ...
        let errors = issues.iter().filter(|i| i.severity == SimIssueSeverity::Error).count();
        let doc = entries(vec![
    ...
    if template_kind {
        ...
        let site = match options.site_id.as_deref() {
            ...
            None => {
                let matched = match_on_map(&file_template.template, map.bundle(), &SiteMatchOptions::default())?;
                match matched.report.sites.first() {
                    Some(site) => site.clone(),
                    None => {
                        return Err(CompileError::at("no_site", "--map", format!("no site matched on {map_id}"))
                            .detail_entry("failureSummary", serde_json::json!(matched.report.failure_summary))
                            .as_findings());
                    }
                }
            }
    ...
    let violated = invariants
        ...
        .count();
    let errors = run.issues.iter().filter(|i| i.severity == SimIssueSeverity::Error).count();
    ...
    let doc = entries(vec![
        ...
        ("triggerNeverFired", js(&run.trace.metrics.trigger_never_fired)?),
