...
fn richmond_fixture_catalog_digests_match_typescript() {
    ...
    let created = create_catalog(
        ...
        &CreateOptions {
            map_ids: Some(vec!["richmond-field-station".into()]),
    ...
    assert_eq!(
        slot(0, "designDigest"),
        "73ee282c02f86556e9c6356f7dbcb3ed09967aecedd68f11707071bdfa96e55f"
    );
    assert_eq!(
        slot(99, "designDigest"),
        "77400549c23256bd7f089bcbf9094f5548e463b8fa5042fd2ddbc641890df738"
    );
    assert_eq!(
        slot(0, "seed"),
        "e1bbe8723c5d5d741bb867ea695e53095b7206c9a77454393c75c7d67663003e"
    );
    let maps = catalog.get("maps").and_then(|m| m.as_array()).unwrap();
    assert_eq!(
        maps[0]
            .get("locationCatalogDigest")
            .and_then(|v| v.as_str()),
