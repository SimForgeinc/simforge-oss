...
|---|---|---|---|---|
| F-01 | High | The live importer mirrored every actor north↔south. It set scene `z = +y_osc`; the scene frame is `z = −y_osc`, and the compiler reads map `y = −z`. The test fixture had pinned the bug. | `probe-world-position-frame` | fixed |
