...
- **LOD.** Per view, each heavy mesh draws at the coarsest level whose geometric error stays under the pixel budget for that view's own focal length, floored at 1000 px (`geometry_lod::REFERENCE_F_PX`: at its own 370 px a 120° 720p camera visibly thinned canopies overhead, because a foliage level's geometric error understates what it drops). The ranges are computed once, and each camera scales its distance (`VisibilityRangeScale`, vendored `bevy_camera`), so a camera's frames never depend on which other cameras the rig has: a dash camera renders byte-identically with and without the trailing chase camera. The distance is measured to the manifest's bounds centre. Ranges are abrupt `VisibilityRange`s with no dither, so a frame is a pure function of the pose. The ID pass clones every level with the same ranges. Lidar and radar keep full detail.
