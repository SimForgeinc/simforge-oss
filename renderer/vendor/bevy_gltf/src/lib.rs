...
//!
//! 1. Native `KHR_texture_basisu` support (bevyengine/bevy#19104): the `gltf`
//!    dependency enables the `allow_empty_texture` feature so validation
//!    accepts required `KHR_texture_basisu` and source-less textures, and the
//!    loader resolves each texture's image through
//!    `extensions.KHR_texture_basisu.source` when the extension is present,
//!    else the core `source` field. Textures referencing no image fail with
//!    `GltfError::MissingTextureSource`; a present-but-malformed extension
//!    source fails with `GltfError::InvalidBasisuSource` instead of silently
//!    falling back to a compatibility image.
//! 2. Color-space classification covers every gated extension slot:
...
//! | `EXT_texture_webp`                | ❌\*      |                                     |
//!
//! \*This fork resolves `KHR_texture_basisu` image sources natively (KTX2 decoding requires the `ktx2` feature, plus `basis-universal` for UASTC/ETC1S transcoding); upstream tracking issue: [#19104](https://github.com/bevyengine/bevy/issues/19104). `EXT_texture_webp` syntax remains unsupported: Bevy decodes webp images, but the loader does not read that extension's `source`.
//!
