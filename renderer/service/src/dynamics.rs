...
impl Bicycle {
    ...
    pub fn integrate(
        ...
        .all(|v| v.is_finite())
            ...
            || wheelbase <= 0.0
            || dt <= 0.0
            || !(-10.0..=5.0).contains(&acceleration)
            || steering.abs() > 0.7
        {
        ...
        let moving_dt = if acceleration < 0.0 {
            dt.min(self.speed / -acceleration)
        } else {
            dt
        };
...
mod tests {
    ...
    fn authored_yale_trajectory_as_actions_reconstructs_world_path() {
        ...
        let rows: Vec<Vec<f64>> = include_str!("../testdata/yale-ego.csv")
            .lines()
            .filter(|s| !s.starts_with('#'))
            .map(|s| s.split(',').map(|n| n.parse().unwrap()).collect())
            .collect();
        let mut state = Bicycle {
            x: rows[0][0],
            z: rows[0][1],
            yaw: rows[0][2],
            speed: rows[0][3],
        ...
        for pair in rows.windows(2) {
            ...
            let steering = if distance > 1e-12 {
                (2.7 * turn / distance).atan()
            } else {
                0.0
            };
            state
                .integrate((b[3] - a[3]) / 0.02, steering, 2.7, 0.02)
                .unwrap();
        ...
        println!(
            "Yale 1000 transitions: max={max_error:.9}m RMS={:.9}m",
            (sum_squared / 1000.0).sqrt()
        );
        assert!(
            max_error < 0.02,
            "world path error {max_error} exceeds 2 cm"
        );
    }
