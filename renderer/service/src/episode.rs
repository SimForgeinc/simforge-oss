...
struct MotionSample {
    index: usize,
    vehicle: crate::dynamics::Bicycle,
    acceleration: [f64; 2],
}
...
impl Episode {
    pub fn reset(
        ...
        for frame in &document.frames {
            ...
            for pose in &frame.actors {
                ...
                if !desc
                    .dims
                    .is_some_and(|d| [d.l, d.w, d.h].iter().all(|v| v.is_finite() && *v > 0.0))
                {
                ...
                actors.push(ActorState {
                    ...
                    kind: match pose.kind {
                        ActorTickKind::Spawn => "spawn",
                        ActorTickKind::Update => "update",
                        ActorTickKind::Despawn => "despawn",
                    }
                    .into(),
                    catalog_id: Some(desc.catalog_id.clone()),
                    actor_class: Some(desc.actor_class.clone()),
                    color: desc.color.clone(),
                    dims: desc.dims.map(|d| ActorDims {
                        l: d.l as f32,
                        w: d.w as f32,
                        h: d.h as f32,
                    }),
                    transform: ActorTransform {
                        position: pose.position.map(|v| v as f32),
                        rotation: pose.rotation.map(|v| v as f32),
                    },
                    velocity: pose.velocity.map(|v| v as f32),
                    wheel_spin_rad,
                    body_attitude: pose.body_attitude.map(|a| crate::scene::BodyAttitude {
                        pitch_rad: a.pitch_rad as f32,
        ...
        let yaw =
            (2.0 * (q[3] * q[1] + q[2] * q[0])).atan2(1.0 - 2.0 * (q[1] * q[1] + q[2] * q[2]));
        let vehicle = crate::dynamics::Bicycle {
            x: ego.transform.position[0] as f64,
            z: ego.transform.position[2] as f64,
            yaw: yaw as f64,
        ...
        Ok(Self {
            ...
            index: 0,
            ego_id,
            stride: (hz / 2.0).round() as usize,
            wheelbase_m: limits.wheelbase_m,
            vehicle,
            replay_divergence_m: 0.0,
            replay_valid: true,
            history: vec![MotionSample {
                index: 0,
                vehicle,
    ...
    pub fn configure_consumer(
        ...
        for rate in [
            consumer.camera_hz,
            consumer.ego_history.hz,
            consumer.future_target.hz,
        ] {
    ...
    pub fn warm_start(&mut self) -> Result<Vec<usize>, String> {
        ...
        if index + 1 >= self.authored.len() {
            return Err(
                "scenario too short for declared observation history and one controlled tick"
                    .into(),
            );
        }
        self.history.clear();
        for i in 0..=index {
            let actor = self.authored[i]
                .actors
                .iter()
                .find(|a| a.id == self.ego_id)
                .unwrap();
            let pose = footprint(actor);
            let previous = self.authored[i.saturating_sub(1)]
                .actors
                .iter()
                .find(|a| a.id == self.ego_id)
                .unwrap();
            ...
            self.history.push(MotionSample {
                index: i,
                vehicle,
                acceleration: [
                    (actor.velocity[0] - previous.velocity[0]) as f64 * hz,
        ...
        self.initial_goal_distance =
            (self.vehicle.x - self.goal[0]).hypot(self.vehicle.z - self.goal[1]);
        Ok((0..self.consumer.camera_history_frames as usize)
            .map(|n| index - (self.consumer.camera_history_frames as usize - 1 - n) * self.stride)
            .collect())
    }
    ...
    pub fn advance(
        &mut self,
        action: Action,
        drivable: impl Fn(crate::traffic::Footprint) -> bool,
    ) -> Result<SceneState, String> {
        if self.termination.is_some() {
            return Err("episode terminated; reset required".into());
        }
        let Action::Bicycle {
            acceleration_mps2,
            steering_rad,
        ...
        for _ in 0..self.stride.min(self.authored.len() - 1 - self.index) {
            ...
            self.vehicle
                .integrate(acceleration_mps2, steering_rad, self.wheelbase_m, dt)?;
            self.index += 1;
            let authored = self.authored[self.index]
                .actors
                .iter()
                .find(|a| a.id == self.ego_id)
                .unwrap();
            ...
            let acceleration = [
                (self.vehicle.speed * self.vehicle.yaw.cos() - previous.speed * previous.yaw.cos())
                    / dt,
                (-self.vehicle.speed * self.vehicle.yaw.sin()
            ...
            let actual = crate::traffic::Footprint {
                x: self.vehicle.x,
                z: self.vehicle.z,
        ...
        ego.transform.rotation = [
            0.0,
            (vehicle.yaw * 0.5).sin() as f32,
            0.0,
            (vehicle.yaw * 0.5).cos() as f32,
        ];
        ego.velocity = [
            (vehicle.speed * vehicle.yaw.cos()) as f32,
            0.0,
            (-vehicle.speed * vehicle.yaw.sin()) as f32,
        ];
        Ok(frame)
    }
    ...
    pub fn evaluate(&mut self, drivable: &impl Fn(crate::traffic::Footprint) -> bool) {
        ...
        } else if let Some(other) = frame.actors.iter().find(|a| {
            a.id != self.ego_id && a.kind != "despawn" && actual.separation(footprint(a)) <= 0.0
        }) {
            Some(Termination::Collision {
                actor_id: other.id.clone(),
            })
        } else if !drivable(actual) {
            Some(Termination::OffRoad)
        } else if (self.vehicle.x - self.goal[0]).hypot(self.vehicle.z - self.goal[1])
            <= self.limits.goal_radius_m
            && self.vehicle.speed <= 0.5
        {
            Some(Termination::GoalReached)
        } else if self.index + 1 == self.authored.len()
            || (self.index - self.start_index) as f64 / frame.tick_hz as f64
    ...
    fn progress(&self) -> f64 {
        if self.initial_goal_distance < 1e-6 {
            return f64::from(matches!(self.termination, Some(Termination::GoalReached)));
        }
        (1.0 - (self.vehicle.x - self.goal[0]).hypot(self.vehicle.z - self.goal[1])
            / self.initial_goal_distance)
            .clamp(0.0, 1.0)
    }
    ...
    fn trajectory(&self, future: bool) -> Trajectory {
        let mut out = Trajectory {
            frame: "actor-origin x-forward/y-left; heading positive left",
            poses: Vec::new(),
            velocity: Vec::new(),
            acceleration: Vec::new(),
            valid: Vec::new(),
            acceleration_valid: Vec::new(),
        };
        let samples = if future {
            self.consumer.future_target
        } else {
            self.consumer.ego_history
        };
        ...
        let offsets = if future {
            1..=samples.points as isize
        } else {
            1 - samples.points as isize..=0
        };
        for offset in offsets {
            ...
            let sample = if index < 0 {
                ...
                self.authored
                    .get(index as usize)
                    .and_then(|frame| frame.actors.iter().find(|a| a.id == self.ego_id))
                    .map(|actor| {
                        let pose = footprint(actor);
                        let previous = self.authored[index as usize - 1]
                            .actors
                            .iter()
                            .find(|a| a.id == self.ego_id)
                            .unwrap();
                ...
                self.history.get(index as usize).copied()
            };
            out.valid.push(sample.is_some());
            out.acceleration_valid
                .push(sample.is_some_and(|s| s.index > 0));
            if let Some(sample) = sample {
                ...
                out.velocity
                    .push(rotate(v.speed * v.yaw.cos(), -v.speed * v.yaw.sin()));
                out.acceleration
                    .push(rotate(sample.acceleration[0], sample.acceleration[1]));
            } else {
                out.poses.push([0.0; 3]);
                out.velocity.push([0.0; 2]);
                out.acceleration.push([0.0; 2]);
            }
        }
        out
    }
    ...
    pub fn observe(&self, frame: &SceneState) -> EpisodeObservation {
        ...
        EpisodeObservation {
            tick: frame.tick,
            time_seconds: frame.tick as f64 / frame.tick_hz as f64,
            ego_id: self.ego_id.clone(),
            position: ego.transform.position,
            rotation: ego.transform.rotation,
            ...
            score: match self.termination {
                Some(Termination::GoalReached) => 100.0,
                Some(
                    Termination::Collision { .. }
                    | Termination::OffRoad
                    | Termination::ReplayInvalid,
                ) => 0.0,
...
pub fn footprint(actor: &ActorState) -> crate::traffic::Footprint {
    ...
    crate::traffic::Footprint {
        x: actor.transform.position[0] as f64,
        z: actor.transform.position[2] as f64,
        yaw: (2.0 * (q[3] * q[1] + q[2] * q[0])).atan2(1.0 - 2.0 * (q[1] * q[1] + q[2] * q[2]))
            as f64,
        length: dims.l as f64,
        width: dims.w as f64,
    }
}
...
mod tests {
    ...
    pub(super) fn fixture(other: bool) -> Document {
        let mut actors = vec![
            serde_json::json!({"id":"ego","catalogId":"car","actorClass":"car","dims":{"l":2.0,"w":1.0,"h":1.0}}),
        ];
        if other {
            actors.push(serde_json::json!({"id":"other","catalogId":"car","actorClass":"car","dims":{"l":2.0,"w":1.0,"h":1.0}}));
        }
        ...
        serde_json::from_value(
            serde_json::json!({"version":"simforge.scene-state.v1","mapId":"fixture",
            "frame":"scene-yup","dt":0.02,"tickHz":50,"tickCount":101,"weather":{"preset":"clear"},
            "timeOfDay":12,"actors":actors,"frames":frames}),
        )
        .unwrap()
    }
    fn straight() -> Action {
    ...
    fn termination_checks_substeps_and_blocks_post_terminal_actions() {
        let mut collision =
            Episode::reset(fixture(true), "ego".into(), EpisodeLimits::default()).unwrap();
        ...
        let mut road =
            Episode::reset(fixture(false), "ego".into(), EpisodeLimits::default()).unwrap();
    ...
    fn goal_and_history_do_not_leak_future_or_invent_past() {
        ...
        for _ in 0..3 {
            episode.advance(straight(), |_| true).unwrap();
        }
        ...
        episode.vehicle.x = 2.0;
        episode.vehicle.speed = 0.0;
...
mod temporal_tests {
    ...
    fn named_consumers_have_distinct_real_history_and_control_cadence() {
        for (consumer, expected, next) in [
            (
                render_core::products::ConsumerSpec::qwen_drive(),
                vec![0, 25, 50, 75],
                100,
            ),
            (
                render_core::products::ConsumerSpec::alpamayo_r1(),
                vec![60, 65, 70, 75],
                80,
            ),
        ] {
            let mut episode = Episode::reset(
                super::tests::fixture(false),
                "ego".into(),
