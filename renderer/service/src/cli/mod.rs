...
pub fn main(argv: Vec<String>) -> anyhow::Result<()> {
    ...
    match command.as_str() {
        ...
        "job" => job::run(rest),
        "view" => simforge_render_view::run(std::iter::once(format!("{program} view")).chain(rest).collect()),
        "dev" => {
            let mut rest = rest.into_iter();
            let tool = rest.next().ok_or_else(|| anyhow::anyhow!("usage: simforge-render dev <parity-check|sky-bench|ktx2-gpu-variant> [args...]"))?;
            let argv: Vec<String> = std::iter::once(format!("{program} dev {tool}")).chain(rest).collect();
            match tool.as_str() {
                ...
                other => anyhow::bail!("unknown dev tool {other:?} (parity-check | sky-bench | ktx2-gpu-variant)"),
            }
        }
        "--help" | "-h" | "help" => {
            println!("{USAGE}");
