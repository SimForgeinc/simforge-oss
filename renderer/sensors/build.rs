...
fn main() {
    ...
    if env::var_os("CARGO_FEATURE_GPU_VIDEO").is_none() {
        return;
    }
    assert_eq!(
        env::var("CARGO_CFG_TARGET_OS").unwrap(),
        "linux",
        "gpu-video requires Linux NVIDIA drivers"
    );
    ...
    assert!(
        Command::new(env::var_os("CC").unwrap_or_else(|| "cc".into()))
            .args([
                ...
                "-fPIC",
                "-c",
                "src/nvcodec/encode.c",
                "-o"
            ])
            .arg(&object)
            .status()
            .expect("compile NVENC bridge")
            .success()
    );
    assert!(
        Command::new(env::var_os("AR").unwrap_or_else(|| "ar".into()))
            .arg("crs")
            .arg(out.join("libsensor_nvenc.a"))
            .arg(object)
            .status()
            .expect("archive NVENC bridge")
            .success()
    );
