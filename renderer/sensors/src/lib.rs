...
pub static RAY_POOL: std::sync::LazyLock<bevy::tasks::TaskPool> = std::sync::LazyLock::new(|| {
    let cores = std::thread::available_parallelism()
        .map(|n| n.get())
        .unwrap_or(4); // fallback-ok: thread-pool size only; results do not depend on it
