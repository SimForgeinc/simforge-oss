...
fn inflate(tri: [Vec3; 3], extra: f32) -> [Vec3; 3] {
    ...
    let outward = |i: usize| -> Vec3 {
        ...
        if out.dot(tri[(i + 2) % 3] - tri[i]) > 0.0 { -out } else { out }
    };
...
fn candidates(tri: [Vec3; 3], margin: f32, plane_error: f32) -> (Vec<[Vec3; 3]>, bool) {
    let longest = (tri[1] - tri[0]).length().max((tri[2] - tri[1]).length()).max((tri[0] - tri[2]).length());
    ...
    let shape = if area2 > 0.0 { longest * longest / area2 } else { f32::INFINITY };
    let pad = margin
        + tri.iter().map(|v| v.abs().max_element()).fold(0.0f32, f32::max) * INFLATE_REL
        + INFLATE_ABS
    ...
    let min = tri.iter().fold(Vec3::splat(f32::INFINITY), |m, v| m.min(*v)) - Vec3::splat(pad);
    let max = tri.iter().fold(Vec3::splat(f32::NEG_INFINITY), |m, v| m.max(*v)) + Vec3::splat(pad);
    let corner = |i: u32| Vec3::new(
...
pub fn debug_margins(scene: &InstancedScene, mesh: usize) -> (f32, f32) {
    ...
    let s = if min_scale[mesh].is_finite() && min_scale[mesh] > 0.0 { min_scale[mesh] } else { 1.0 };
    let overshoot = OVERSHOOT_FACTOR * f32::EPSILON * world_radius.max(1.0) / crate::bvh::MIN_INCIDENCE_COS;
    (overshoot / s, SLIVER_FACTOR * f32::EPSILON * world_radius.max(1.0) / s)
}
        ...
        for x in [min.x, max.x] {
            for y in [min.y, max.y] {
                for z in [min.z, max.z] {
                    world_radius = world_radius.max(world.transform_point3(Vec3::new(x, y, z)).abs().max_element());
                }
            }
        }
...
impl GpuRayScene {
    ...
    pub fn new(scene: &InstancedScene, device: wgpu::Device, queue: wgpu::Queue) -> anyhow::Result<Self> {
        let started = std::time::Instant::now();
        anyhow::ensure!(supported(&device), "GPU ray scene: the device has no hardware ray-query support");
        ...
        for (mesh, tris) in scene.gpu_meshes().enumerate() {
            ...
            tri_total += tris.len();
            let scale = if min_scale[mesh].is_finite() && min_scale[mesh] > 0.0 { min_scale[mesh] } else { 1.0 };
            ...
            for (group, faces) in faces_by_group.iter().enumerate() {
                entry[group] = ((prim_map.len() / 4) as u32, (inflated.len() / 16) as u32, faces.len());
        ...
        let sizes: Vec<[wgpu::BlasTriangleGeometrySizeDescriptor; 2]> =
            groups.iter().map(|g| [size_of(g[0].2), size_of(g[1].2)]).collect();
                    ...
                    (g[group].2 > 0).then(|| {
                        device.create_blas(
                            &wgpu::CreateBlasDescriptor {
                                label: Some(if group == 0 { "gpu rays mesh" } else { "gpu rays envelopes" }),
                                flags: wgpu::AccelerationStructureFlags::PREFER_FAST_TRACE,
                                update_mode: wgpu::AccelerationStructureUpdateMode::Build,
                            },
                            wgpu::BlasGeometrySizeDescriptors::Triangles { descriptors: vec![size[group].clone()] },
                        )
                    })
                })
            })
            .collect();
        anyhow::ensure!(records.len() < (1 << 24), "GPU ray scene: {} instances exceed the 24-bit custom index", records.len());
        ...
        for (index, &(mesh, world, instance_id, order)) in records.iter().enumerate() {
            ...
            for (group, blas) in blases[mesh].iter().enumerate() {
                if let Some(blas) = blas {
                    let mask = if group == 0 { MASK_PLAIN } else { MASK_ENVELOPE };
                    tlas[slot] = Some(wgpu::TlasInstance::new(blas, rows[..12].try_into().unwrap(), index as u32, mask));
                    slot += 1;
                }
            }
            ...
            for value in [first_vertex[mesh], instance_id, order, groups[mesh][0].0, groups[mesh][1].0, 0, 0, 0] {
                instance_bytes.extend_from_slice(&value.to_le_bytes());
            }
        }
        ...
        let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("gpu rays build") });
    ...
    pub fn cast_batch(&self, rays: &[Ray]) -> anyhow::Result<Vec<Option<RawHit>>> {
        let records: Vec<[f32; 8]> = rays
            .iter()
            .map(|r| [r.origin.x, r.origin.y, r.origin.z, r.t_max, r.dir.x, r.dir.y, r.dir.z, 0.0])
            .collect();
        Ok(self
            .run(&self.pipeline, &records)?
            .into_iter()
            .map(|w| (w[0] != 0).then(|| RawHit { instance: w[0] - 1, triangle: w[1], t_bits: w[2] }))
            .collect())
    }
    ...
    pub fn debug_exact(&self, queries: &[(Ray, u32, u32)]) -> anyhow::Result<Vec<(bool, u32)>> {
        ...
        let records: Vec<[f32; 8]> = queries
            ...
            .map(|(r, instance, triangle)| {
                [r.origin.x, r.origin.y, r.origin.z, f32::from_bits(*triangle), r.dir.x, r.dir.y, r.dir.z, f32::from_bits(*instance)]
            })
            .collect();
        Ok(self.run(&pipeline, &records)?.into_iter().map(|w| (w[0] != 0, w[1])).collect())
    }

    fn run(&self, pipeline: &wgpu::ComputePipeline, records: &[[f32; 8]]) -> anyhow::Result<Vec<[u32; 4]>> {
        if records.is_empty() {
            return Ok(Vec::new());
        }
        let groups = (records.len() as u32).div_ceil(64);
        anyhow::ensure!(groups <= 65535, "GPU ray batch of {} rays is too large for one dispatch", records.len());
        ...
        if std::ptr::eq(pipeline, &self.pipeline) {
            entries.push(wgpu::BindGroupEntry { binding: 6, resource: self.prims.as_entire_binding() });
        }
        if std::ptr::eq(pipeline, &self.pipeline) {
            entries.insert(0, wgpu::BindGroupEntry { binding: 0, resource: self.tlas.as_binding() });
        }
        ...
        let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("gpu rays trace") });
        {
        ...
        let out = mapped
            .chunks_exact(16)
            .map(|record| std::array::from_fn(|i| u32::from_le_bytes(record[i * 4..i * 4 + 4].try_into().unwrap())))
            .collect();
        drop(mapped);
        staging.unmap();
        Ok(out)
    ...
    pub fn cast_hits(&self, scene: &InstancedScene, rays: &[Ray]) -> anyhow::Result<Vec<Option<Hit>>> {
        Ok(self
            .cast_batch(rays)?
            .into_iter()
            .zip(rays)
            .map(|(raw, ray)| {
                raw.map(|raw| scene.hit_from(raw.instance as usize, raw.triangle as usize, ray.origin, ray.dir, raw.t()))
            })
            .collect())
    }
}
...
mod tests {
    ...
    fn inflation_contains_the_original_triangle_with_margin() {
        let tris = [
            ...
            // A sliver.
            [Vec3::new(100.0, 2.0, 5.0), Vec3::new(130.0, 2.0, 5.0), Vec3::new(115.0, 2.001, 5.0)],
            // Tilted, far from the origin.
            [Vec3::new(-250.0, 10.0, 80.0), Vec3::new(-249.0, 11.0, 80.5), Vec3::new(-250.5, 10.2, 81.0)],
        ];
        for tri in tris {
            ...
            // Same plane.
            for v in big {
                assert!((v - tri[0]).dot(n).abs() < 1e-4, "{v:?} left the plane of {tri:?}");
            }
            ...
            for v in tri {
                assert!(bary(v).iter().all(|w| *w > 0.0), "{v:?} not strictly inside {big:?}");
            }
        }
    }
}
