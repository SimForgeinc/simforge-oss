...
fn meshes(rng: &mut Rng) -> Vec<Vec<Tri>> {
    ...
    for i in 0..n {
        for j in 0..n {
            let p = |x: usize, z: usize| {
                Vec3::new(
                    x as f32 * 2.0 - 24.0,
                    ((x * 7 + z * 3) % 5) as f32 * 0.05,
                    z as f32 * 2.0 - 24.0,
                )
            };
...
fn scene(rng: &mut Rng, at_scale: f32) -> (InstancedScene, Vec<Tri>) {
    ...
    let blases: Vec<Blas> = meshes
        .iter()
        .map(|tris| Blas::build(tris.iter().copied()))
        .collect();
    ...
    for k in 0..30u32 {
        let world = Mat4::from_scale_rotation_translation(
            ...
            Quat::from_rotation_y(rng.range(0.0, 6.28))
                * Quat::from_rotation_x(rng.range(-0.2, 0.2)),
            Vec3::new(
                at_scale + rng.range(-20.0, 20.0),
                rng.range(1.0, 6.0),
                -at_scale + rng.range(-20.0, 20.0),
            ),
        );
    ...
    for i in 0..count {
        ...
        let target = match i % 5 {
            ...
            4 => {
                ...
                let dir =
                    Vec3::new(rng.range(-1.0, 1.0), 0.0, rng.range(-1.0, 1.0)).normalize_or_zero();
...
fn hardware_rays_match_the_cpu_reference_bit_for_bit() {
    ...
    let extra: u64 = std::env::var("SIMFORGE_GPU_RAYS_SEEDS")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(0);
    let mut cases = vec![
        (0x9e3779b97f4a7c15u64, 0.0f32),
        (0x2545f4914f6cdd1d, 300.0),
        (0xda942042e4dd58b5, 1200.0),
    ];
    for k in 0..extra {
        cases.push((
            0x5851f42d4c957f2d ^ (k + 1).wrapping_mul(0x9e3779b97f4a7c15),
            [0.0, 80.0, 450.0, 2500.0][k as usize % 4],
        ));
    }
    for (seed, at_scale) in cases {
            ...
            let same = match (&want, got) {
                ...
                (Some(w), Some(g)) => {
                    w.distance.to_bits() == g.distance.to_bits()
                        ...
                        && w.normal.to_array().map(f32::to_bits)
                            == g.normal.to_array().map(f32::to_bits)
                        && w.point.to_array().map(f32::to_bits)
                            == g.point.to_array().map(f32::to_bits)
                }
                _ => false,
            };
        ...
        if !mismatches.is_empty() {
            ...
            let raw = gpu
                .cast_batch(&queries.iter().map(|q| q.0).collect::<Vec<_>>())
                .expect("raw");
            for ((query, (accepted, t_bits)), raw) in queries.iter().zip(&exact).zip(&raw).take(12)
            {
                let cpu = scene
                    .cast_indexed(query.0.origin, query.0.dir, query.0.t_max)
                    .unwrap();
                ...
                let cosi = det / (e1.cross(e2).length() * d.length());
                {
                    ...
                    let (faces, envelope) = sensors::gpu_rays::debug_candidates(
                ...
                eprintln!(
                    "   f64: t {t:.6} u {u:.6} v {v:.6} cos {cosi:.4} edges {:.3} {:.3} {:.3}",
                    (b - a).length(),
                    (c - b).length(),
                    (a - c).length()
                );
                {
                    ...
                    let t = e2.dot(qv) * inv;
                    eprintln!(
                        "   f32: t {t:.6} u {u:.6} v {v:.6} det {det:e} a {a:?} o {o:?} d {d:?}"
                    );
                }
                eprintln!(
