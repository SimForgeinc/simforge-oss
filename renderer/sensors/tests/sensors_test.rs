...
fn binary_ply_carries_the_same_points_as_the_ascii_form() {
    ...
    assert_eq!(
        body.len(),
        points.len() * 20,
        "5 x 4-byte properties per point"
    );
...
fn composite_scene_returns_the_nearest_layer_hit() {
    ...
    let hit =
        sensors::bvh::Raycast::cast(&composed, Vec3::new(-100.0, 5.0, 90.0), Vec3::NEG_Y, 100.0)
            .expect("hit");
...
fn bvh_nearest_hit_and_normal() {
    let s = ground_scene();
    let hit = s
        .cast(Vec3::new(1.0, 3.0, 2.0), Vec3::NEG_Y, 100.0)
        .expect("hit");
...
fn lidar_scan_is_ordered_and_deterministic() {
    ...
    for p in &a {
        assert!(
            (p.y + 2.0).abs() < 1e-3,
            "ground return sits 2 m below the sensor"
        );
