...
fn layout(format: TextureFormat) -> Option<(VariantCodec, u32, u8, &'static [(u16, u8, u8)], u32)> {
    match format {
        ...
        TextureFormat::Bc5RgUnorm => Some((VariantCodec::Bc5, 141, 132, &[(0, 63, 0), (64, 63, 1)], 16)),
...
pub fn gpu_variant(source: &[u8], supercompression: Supercompression) -> Result<GpuVariant> {
    let reader = ktx2::Reader::new(source).map_err(|error| anyhow::anyhow!("not a KTX2 file: {error:?}"))?;
    let header = reader.header();
    let (width, height, levels) = (header.pixel_width, header.pixel_height, header.level_count.max(1));
    if header.format.is_some() {
        return Ok(GpuVariant { codec: VariantCodec::Passthrough, ktx2: None, vk_format: 0, width, height, levels });
    }
    ensure!(
        ...
        width, height, header.pixel_depth, header.layer_count, header.face_count
    );
    let image = ktx2_buffer_to_image(source, CompressedImageFormats::BC, false).context("transcode (Bevy loader path)")?;
    ...
    let data = image.data.as_ref().context("transcoded image has no data")?;
    ...
    for level in 0..levels {
        ...
        ensure!(offset + bytes <= data.len(), "transcoded data is shorter than its {levels} levels");
        level_data.push(&data[offset..offset + bytes]);
        offset += bytes;
    }
    ensure!(offset == data.len(), "transcoded data has {} bytes beyond its levels", data.len() - offset);
    let ktx2 = write_ktx2(vk_format, color_model, samples, block_bytes, width, height, &level_data, supercompression);
    // Verify: the variant loads to exactly what the source loads to.
    for srgb in [false, true] {
        let from_source = if srgb { ktx2_buffer_to_image(source, CompressedImageFormats::BC, true)? } else { image.clone() };
        let from_variant = ktx2_buffer_to_image(&ktx2, CompressedImageFormats::BC, srgb).context("load the variant")?;
        ensure!(
            from_variant.texture_descriptor.format == from_source.texture_descriptor.format
                ...
                && from_variant.texture_descriptor.mip_level_count == from_source.texture_descriptor.mip_level_count
                && from_variant.data == from_source.data,
            "variant does not load to the source's blocks (srgb={srgb})"
        );
    }
    Ok(GpuVariant { codec, ktx2: Some(ktx2), vk_format, width, height, levels })
}
...
fn write_ktx2(
    ...
    let stored: Vec<Vec<u8>> = levels
        ...
        .map(|level| match supercompression {
            ...
            Supercompression::ZstdFastest => {
                ruzstd::encoding::compress_to_vec(*level, ruzstd::encoding::CompressionLevel::Fastest)
            }
        })
        .collect();
    ...
    let align = if supercompression == Supercompression::None { 16 } else { 1 };
    let mut out = Vec::new();
    ...
    for value in [
        ...
        1, // faces
        level_count as u32,
        if supercompression == Supercompression::None { 0 } else { 2 },
        dfd_offset as u32,
        dfd.len() as u32,
        kvd_offset as u32,
        kvd.len() as u32,
...
mod tests {
    ...
    fn every_closure_texture_has_a_byte_identical_variant() {
        ...
        let totals = std::sync::Mutex::new((0usize, 0usize, std::collections::BTreeMap::<&str, usize>::new()));
    ...
    fn writes_a_parseable_ktx2_with_levels_smallest_first() {
        ...
        for compression in [Supercompression::None, Supercompression::ZstdFastest] {
            ...
            assert_eq!(image.data.unwrap(), [levels[0].clone(), levels[1].clone()].concat(), "{compression:?}");
        }
    }
}
