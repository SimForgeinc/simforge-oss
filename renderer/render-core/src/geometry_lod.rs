...
impl Manifest {
    pub fn load(path: &std::path::Path) -> Result<Self> {
        let manifest: Manifest = serde_json::from_slice(
            &std::fs::read(path)
                .with_context(|| format!("read geometry LOD manifest {}", path.display()))?,
        )
...
fn range_for(member: &LodMember, scale: f32, f_px: f32, pixel_error_px: f32) -> VisibilityRange {
    ...
    let end = member.end_error_m.map_or(f32::INFINITY, |error| {
        switch_distance(error, scale, f_px, pixel_error_px)
    });
    VisibilityRange {
...
fn bounds_aabb(
    ...
    (
        bevy::camera::primitives::Aabb::from_min_max(
            center - Vec3::splat(bounds.radius),
            center + Vec3::splat(bounds.radius),
        ),
        bevy::camera::visibility::NoAutoAabb,
    )
...
pub(crate) fn spawn_levels(
    ...
    let by_mesh: HashMap<u32, &ManifestMesh> =
        lods.manifest.meshes.iter().map(|m| (m.mesh, m)).collect();
    ...
    let mut masters: Vec<(Entity, u32, u32)> = Vec::new();
    {
        ...
        for (entity, mesh, _) in query.iter(world) {
            ...
            let Some(label) = path.strip_prefix(&prefix) else {
                continue;
            }; // fallback-ok: other files have no LODs
            let Some((mesh_index, primitive)) = mesh_primitive(label) else {
                continue;
            }; // fallback-ok: non-primitive sub-assets
    ...
    for (master, mesh_index, primitive) in &masters {
        ...
        let first_error = entry
            .levels
            .first()
            .map(|l| l.geometric_error_m)
            .or(entry.impostor.as_ref().map(|i| i.geometric_error_m));
        let bound = bounds_aabb(&entry.bounds);
        world.entity_mut(*master).insert((
            LodMember {
                start_error_m: 0.0,
                end_error_m: first_error,
            },
            bound.clone(),
        ));
        if let Some((clone, _)) = id_clones.get(master) {
            world.entity_mut(*clone).insert((
                LodMember {
                    start_error_m: 0.0,
                    end_error_m: first_error,
                },
        ...
        for (k, level) in entry.levels.iter().enumerate() {
            ...
            chain.push((
                asset_server.load(format!(
                    "{}#Mesh{}/Primitive{}",
                    lods.lod_path, level.lod_mesh, primitive
                )),
                material.clone(),
                level.geometric_error_m,
                end,
        ...
        if let (Some(impostor), 0) = (&entry.impostor, *primitive) {
            chain.push((
                asset_server.load(format!(
                    "{}#Mesh{}/Primitive0",
                    lods.lod_path, impostor.lod_mesh
                )),
...
pub(crate) fn apply_ranges(world: &mut World, f_px: f32, pixel_error_px: f32) {
    ...
    let updates: Vec<(Entity, VisibilityRange)> = query
        ...
        .map(|(entity, member, global)| {
            ...
            (
                entity,
                range_for(member, scale.abs().max_element(), f_px, pixel_error_px),
            )
        })
        .collect();
...
mod tests {
    ...
    fn impostor_material_loads_the_standard_material_sub_asset() {
        assert_eq!(
            impostor_material_path("m/lod.gltf", 3),
            "m/lod.gltf#Material3/std"
        );
    }
    ...
    fn labels_parse_and_ranges_tile_the_distance_axis() {
        ...
        let ranges: Vec<VisibilityRange> = members
            .iter()
            .map(|m| range_for(m, 0.4, f_px, 1.0))
            .collect();
