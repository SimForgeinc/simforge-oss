...
impl Preset {
    ...
    pub fn parse(name: &str) -> Result<Self> {
        Self::ALL
            ...
            .ok_or_else(|| {
                anyhow::anyhow!(
                    "[native_render_preset_unknown] preset {name:?} (training | showcase)"
                )
            })
    }
}
...
impl SensorConfig {
    ...
    pub fn ev100_range(&self) -> (f32, f32) {
        ...
        (
            (n2 / self.max_shutter_s).log2() - (self.iso_max / 100.0).log2(),
            (n2 / self.min_shutter_s).log2(),
        )
    }
}
...
impl RenderConfig {
    ...
    pub fn preset(preset: Preset) -> Self {
        ...
        let base = Self {
            ...
            bloom: BloomConfig {
                intensity: look.bloom_intensity,
            },
            dof: DofConfig {
            ...
            motion_blur: MotionBlurConfig {
                shutter_angle: look.motion_shutter_angle,
                samples: look.motion_samples,
            },
            ...
            camera: CameraConfig {
                ...
                sensor: SensorConfig {
                    f_number: 1.8,
                    min_shutter_s: 1.0 / 32000.0,
                    max_shutter_s: 1.0 / 30.0,
                    iso_max: 6400.0,
                },
                wdr: WdrConfig {
                    white_stops: 5.5,
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.output.video.quality > 51 {
            return bad(format!(
                "output.video.quality {} (0..=51)",
                self.output.video.quality
            ));
        }
        if self.encode.finish_threads > 64 {
            return bad(format!(
                "encode.finishThreads {} (0..=64)",
                self.encode.finish_threads
            ));
        }
        let exposure = &self.camera.exposure;
        if !(-6.0..=6.0).contains(&exposure.compensation_ev) {
            return bad(format!(
                "camera.exposure.compensationEv {} (-6..=6)",
        ...
        if !(0.7..=32.0).contains(&sensor.f_number)
            || !(sensor.min_shutter_s > 0.0
                && sensor.min_shutter_s < sensor.max_shutter_s
                && sensor.max_shutter_s <= 1.0)
        ...
        if !(2.0..=12.0).contains(&wdr.white_stops) || !(0.05..=0.5).contains(&wdr.mid_grey) {
            return bad(format!(
                "camera.wdr whiteStops {} midGrey {} (2..=12, 0.05..=0.5)",
                wdr.white_stops, wdr.mid_grey
            ));
        }
        if self.grading.tone_map == ToneMap::DashcamWdr
            && (self.grading.temperature != 0.0 || self.grading.tint != 0.0)
        {
        ...
        if !(0.0..=20.0).contains(&self.atmosphere.haze_density) {
            return bad(format!(
                "atmosphere.hazeDensity {} (0..=20)",
                self.atmosphere.haze_density
            ));
        }
        self.cinematic_fx()
            .validate()
            .context("[native_render_config_invalid]")?;
        Ok(())
    }
...
impl RenderRequest {
    ...
    pub fn push_set(&mut self, assignment: &str) -> Result<()> {
        let (key, value) = assignment.split_once('=').with_context(|| {
            format!("[native_render_config_invalid] --set {assignment:?} is not key=value")
        })?;
        let value = serde_json::from_str(value)
            .unwrap_or_else(|_| serde_json::Value::String(value.to_string())); // fallback-ok: an unquoted value is the string it spells; the typed deserialize below rejects a wrong type
        ...
        for (key, new) in &self.set {
            ...
            for part in key.split('.') {
                slot = slot
                    ...
                    .ok_or_else(|| {
                        anyhow::anyhow!(
                            "[native_render_config_unknown_key] {key:?} is not a render config key"
                        )
                    })?;
            }
            *slot = new.clone();
...
mod tests {
    ...
    fn overrides_apply_and_unknown_keys_fail() {
        ...
        for bad in ["shadows.cascadez=2", "nope=1", "shadows.cascades.x=1"] {
            ...
            assert!(
                error.contains("native_render_config_unknown_key"),
                "{bad}: {error}"
            );
        }
        for bad in [
    ...
    fn every_key_is_listed() {
        let keys: Vec<String> = RenderConfig::preset(Preset::Showcase)
            .keys()
            .unwrap()
            .into_iter()
            .map(|k| k.0)
