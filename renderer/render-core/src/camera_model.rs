...
use bevy::render::render_resource::{
    ...
    BufferUsages, CachedComputePipelineId, CachedRenderPipelineId, ColorTargetState, ColorWrites,
...
impl ExtractComponent for CameraModel {
    ...
    fn extract_component(
        (model, exposure): (&CameraModel, Option<&Exposure>),
    ) -> Option<ExtractedCameraModel> {
...
impl Plugin for CameraModelPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.add_plugins((
            ExtractComponentPlugin::<CameraModel>::default(),
            ExtractComponentPlugin::<HdrCapture>::default(),
        ));
...
fn init_pipelines(
    ...
    let histogram =
        pipeline_cache.queue_compute_pipeline(compute("camera_model_histogram", "histogram_pass"));
    let average =
        pipeline_cache.queue_compute_pipeline(compute("camera_model_average", "average_pass"));
...
fn prepare_apply_pipelines(
    ...
    let Some(mut pipelines) = pipelines else {
        return;
    };
    for target in &views {
...
fn prepare_uniforms(
    ...
    for (entity, extracted, target) in &views {
        ...
        let gpu = GpuCameraParams {
            ...
            misc: Vec4::new(
                m.grading_exposure_ev,
                HISTOGRAM_MIN_LOG2,
                HISTOGRAM_RANGE_LOG2,
                size.width as f32,
            ),
