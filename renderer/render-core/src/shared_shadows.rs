...
fn union_cascades(
    ...
    cameras: Query<(&GlobalTransform, &Projection, &Camera, Option<&RenderLayers>), With<SharedShadowView>>,
    mut lights: Query<
        (&GlobalTransform, &CascadeShadowConfig, &mut Cascades, Option<&RenderLayers>),
...
fn suppress_duplicate_passes(
    mut views: Query<(&ExtractedCamera, &mut bevy::pbr::ViewLightEntities, Has<SharedShadowView>)>,
) {
...
mod tests {
    ...
    fn suppression_stops_at_an_interleaved_unmarked_view() {
        ...
        let v = [(0, false, true), (10, true, true), (20, true, true), (30, true, true)];
        ...
        let v = [(0, true, true), (10, true, true), (20, false, true), (30, true, true)];
        ...
        for (cascade, &far) in cascades.iter().zip(&config.bounds) {
            for (world_from_camera, projection) in &rig {
                for corner in projection.get_frustum_corners(-near, -far) {
                    ...
                    assert!(clip.x.abs() <= 1.0 + 1e-4 && clip.y.abs() <= 1.0 + 1e-4, "{clip:?}");
    ...
    fn cascade_centre_is_texel_snapped() {
        ...
        for cascade in &a {
            ...
            assert!((texels - texels.round()).abs().max_element() < 1e-2, "{texels:?}");
        }
    }
    ...
    fn wide_rig_union_stays_within_thirty_percent_of_one_wide_view() {
        let world_from_light = Transform::default()
            ...
            .to_matrix();
        let config = config();
        let single = union_cascades_for(world_from_light, &config, 2048, &[camera_at(0.0)]).unwrap();
        let rig: Vec<_> = [0.0, 50.0, -50.0, 90.0, -90.0, 140.0, -140.0]
            .iter()
            .map(|y| camera_at(*y))
            .collect();
        let union = union_cascades_for(world_from_light, &config, 2048, &rig).unwrap();
        let ratios: Vec<f32> = single.iter().zip(&union).map(|(one, all)| all.texel_size / one.texel_size).collect();
