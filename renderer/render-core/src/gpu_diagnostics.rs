...
fn timer<'a>(
    slot: &'a mut Option<FrameTimer>,
    device: &RenderDevice,
    queue: &RenderQueue,
) -> Option<&'a mut FrameTimer> {
    if slot.is_none() {
...
fn frame_timer_begin(
    ...
    let Some(t) = timer(&mut shared.0, &device, &queue) else {
        return;
    };
        ...
        if result.is_ok() {
            ...
            if end > begin {
                times
                    .lock()
                    .unwrap()
                    .push((end - begin) as f64 * period / 1.0e6);
            }
        }
        slice_buffer.unmap();
