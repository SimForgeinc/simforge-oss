...
export function DigitalTwinViewerPanel({
  ...
  if (has3D === null) {
    return (
      <div {...stylex.props(styles.viewerLoadingContainer)}>
        <Loader2 {...stylex.props([motionRecipe.spin, styles.loadingSpinner])} />
      </div>
    );
  }
  ...
  return (
    <div {...stylex.props(styles.viewerHostContainer)}>
      ...
      />
      {viewer ? <EditorSceneEnvironmentBridge active={mapGeneration > 0} document={null} quality={renderingPreferenceQuality(quality)} viewer={viewer} /> : null}
    </div>
  );
}
