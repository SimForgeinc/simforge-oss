...
async function loadRegisteredMap(mapVersionId: string): Promise<RegisteredMap | null> {
  ...
  if (!map) return null;
  const [browserRows, nativeRows] = await Promise.all([queryRows<MemberRow>(
  ...
  const rows = await queryRows<SummaryRow>(
    `SELECT mv.id,
     ...
     LEFT JOIN LATERAL (
       ...
       JOIN simforge.browser_asset_blobs b ON b.id = m.blob_id AND b.verification_state = 'verified'
       WHERE m.asset_set_id = bs.id AND m.relative_path = '3d/manifest.json'
       LIMIT 1
     ) browser ON TRUE
     LEFT JOIN LATERAL (
       SELECT jsonb_build_object(
       ...
       JOIN simforge.native_map_asset_blobs b ON b.id = m.blob_id AND b.verification_state = 'verified'
       WHERE m.asset_set_id = ns.id AND m.relative_path = 'master.gltf'
       LIMIT 1
     ) semantic ON TRUE
