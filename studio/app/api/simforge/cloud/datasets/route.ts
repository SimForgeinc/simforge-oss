...
/** `GET /api/simforge/cloud/datasets?organizationId=` -> `{ datasets: ScenarioDatasetDto[] }` from SimCloud. */
export async function GET(request: Request) {
  ...
  try {
    return NextResponse.json(
      { datasets: await listCloudDatasets(organizationId, request.signal) },
