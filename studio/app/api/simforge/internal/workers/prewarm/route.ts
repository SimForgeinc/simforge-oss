...
import { approvedRenderWorker, listPrewarmSets, workerPrewarmFeatures } from "@/app/lib/scenario/workers-prewarm-store";
...
export async function POST(request: Request) {
  ...
  if (!await approvedRenderWorker(workerNodeId)) return NextResponse.json({ error: "worker_not_approved" }, { status: 403 });
  // Newer set fields go only to a worker that declared it parses them.
  return NextResponse.json(await listPrewarmSets(await workerPrewarmFeatures(workerNodeId)), { headers: { "cache-control": "no-store" } });
}
