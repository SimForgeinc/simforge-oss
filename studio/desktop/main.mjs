...
// browser serves. On launch it attaches to an already running local host if
// one exists, otherwise starts the local host supervisor with Electron in
// Node mode (no global Node, pnpm, or tsx) and waits for
...
/** Static pages ship beside this file (asar when packaged). */
...
async function attachOrLaunchHost() {
  ...
  supervisor.once("exit", (code) => {
    ...
    if (window && !window.isDestroyed()) {
      window.loadFile(join(pagesDir, "host-exited.html"), { query: { code: String(code ?? "unknown") } });
    }
  });
      ...
      submenu: [
        {
          ...
          click: () => void shell.openPath(localHostStateDir()),
        },
        {
          label: "Host capabilities",
          click: () => void shell.openExternal(`${baseUrl}/api/simforge/host/capabilities`),
        },
      ],
    },
  ]));
...
if (!app.requestSingleInstanceLock()) {
  ...
  app.whenReady().then(async () => {
    ...
    try {
      const host = await attachOrLaunchHost();
      if (!win.isDestroyed()) await win.loadURL(`${host.baseUrl}/dashboard/scenario`);
    } catch (error) {
      dialog.showErrorBox("SimForge Studio could not start", error instanceof Error ? error.message : String(error));
      app.quit();
    }
  });
  ...
  app.on("before-quit", (event) => {
    ...
    quitting = true;
    void stopOwnedHost().finally(() => app.quit());
  });
}
