...
def test_side_mount_inside_a_wider_host_body_is_pushed_to_its_flank(monkeypatch) -> None:
    ...
    # So is a rear-facing one at 140 deg: it looks past the bodywork.
    assert backend._clear_of_body(host, requested, {"x": -1.0, "y": 0.38, "z": 1.33, "yaw": 140})[1] == 0.38
    # And so is the 50 deg corner camera, whose pose the preset pins.
    assert backend._clear_of_body(host, requested, {"x": 2.0, "y": 0.42, "z": 1.43, "yaw": 50})[1] == 0.42
