...
| Sensor-shaped policy input | `native/crates/simforge-session/src/observation.rs:258-334`: range/FOV gating and LOS against static/actor OBBs, but **LOS-false actors are still appended**. Slabs are `[range_m,bearing_rad,range_rate_mps,los,valid]`; `_native.pyi:279-288`. These are not class/box/covariance observations. |
