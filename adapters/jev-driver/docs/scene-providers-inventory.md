...
- Rust sensor crate exports deterministic CPU BVH, lidar, radar, IMU/GNSS, formats: `renderer/sensors/src/lib.rs:7-22`. Capture CLI covers RGB/depth/instance/semantic cameras, lidar, radar, IMU/GNSS: `renderer/sensors/src/bin/sensor_capture.rs:1-10`.
...
- Native resident service build: `cd renderer && cargo build --release -p service --bin native-render-service`, as documented in `scripts/bench/native-render/README.md:33-36`. Requires native renderer graphics runtime and resolved scene assets, unlike CPU policy execution.
- Rust sensor-capture is explicitly displayless: no primary window, disabled Winit/Audio, zero-duration schedule runner, wgpu render plugin. `renderer/sensors/src/capture.rs:225-240`. CLI requires `--rig-program`, `--glbs` absolute comma-separated GLBs, `--out`; optional scene-state/XODR/tick/size/warmup at `capture.rs:62-86`. GPU/wgpu render device remains necessary for its camera path; this scout did not verify backend availability or whether software GPU fallback is supported.
