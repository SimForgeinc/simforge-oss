...
ROS 2 bridge for the SimForge deterministic sim: clock/TF/odometry out,
...
`scripts/autoware_episode.sh [RUN_DIR]` is the Autoware end-to-end test: a healthy
...
`requirements.txt` pins the gym wheel version. Until that version is on PyPI,
build it from this checkout into the same interpreter instead:
`cd adapters/gym && maturin develop` (Rust toolchain required), then
`pip install -e adapters/gym`.

Not covered: the full Autoware planning stack on a lanelet2 map, and sensor
topics.
